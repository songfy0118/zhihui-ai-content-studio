...
- 机器之心与雷峰网只登记公开文章入口，供人工选择第二来源：机器之心免费 RSS 入口会跳转到数据服务申请/登录，雷峰网未确认官方 RSS，因此两者都不进入自动采集、不抓正文，也不使用 RSSHub 等第三方镜像绕过站点边界
...
- 控制台评分若没有合格的跨来源候选，会复用同一批 RSS 和聚类结果生成临时补证线索；不增加第二轮抓取、不自动搜索、不保存人工选择，也不降低双来源门槛
- 临时补证清单默认每个来源最多占 3 个名额，并显示实际来源数；账号匹配与时效排序仍优先，但单一发布者不能占满全部 12 个候选
- 补证清单同时保留候选池中的中文和英文覆盖，避免中文账号的真实信源被高分英文来源完全挤出；这只改变人工补证入口，不降低双来源门槛
- 默认账号画像显式覆盖来源目录中的 `ai_media`、`tech_media` 和 `company_technology`；评分响应同时报告本轮分类覆盖率，新增分类未映射时不会静默隐藏
- 补证线索按标题字符确定性生成查询语言：中文使用“独立报道 / 核实”，英文使用 `independent coverage verification`；只生成供人工复核的检索词，不自动发起搜索
...
- 检索计划生成后，控制台会按所选标题列出“仅人工打开”的公开新闻页直达入口；点击只会在新标签页打开，不会自动搜索、读取正文、保存证据或解锁草稿
- 第二来源计划会分别统计 `rss_metadata_only` 与 `manual_public_page_review`，并把采集模式纳入计划指纹；公开新闻页不会被“检索公开 RSS 元数据”按钮自动请求
- 第二来源计划在执行前按来源 ID 和精确规范化主机名排除同源入口，并返回排除诊断；不同子域或机构关系仍需人工判断，不据此声称来源独立
- 人工证据审查强制确认同事件、独立采编责任、同集团或合作关系、转载/通稿/引用链、时间线与关键事实冲突共六项；主机不同只是自动预检，不替代人工独立性判断
- 元数据候选对比同时显示原来源与候选的公开链接、规范化主机、发布时间差和共同标题词项；这些仅用于人工筛选，不构成事实一致或来源独立的自动结论
...
- `preflightSourceLockSaveExecution` 只验证票据时窗、消费次数、存储状态、D1 绑定和执行适配器/路由观测；即使全部模拟为就绪，也只允许进入下一次显式执行授权，预检自身不消费票据、不读取或写入数据库
...
- `npm run news:platform-text-creator-draft-save:execute:isolated`：用注入式模拟适配器验证保存草稿契约、同源页面、可见账号及字段/素材指纹，并诚实报告部分保存或异常发布；仓库未接真实浏览器或路由，不会保存真实平台草稿
