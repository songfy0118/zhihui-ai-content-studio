  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      {view === "production" && <section className="panel">
        ...
        {localDraftPreviews.length>0&&<section className="localDraftPreview"><header><div><small>DOUYIN DRAFT · LOCAL PROTOTYPE</small><b>本地抖音草稿雏形已生成</b></div><span>未写入抖音 · 未发布</span></header>{localDraftPreviews.map((draft)=><article key={draft.ideaId}><div><small>标题</small><h3>{draft.title}</h3><p>{draft.body}</p><div>{draft.hashtags.map((tag)=><span key={tag}>#{tag}</span>)}</div></div><aside><small>模拟播放区间</small><b>{draft.estimatedViews}</b><em>等待真实账号指标校准</em><strong>下一步：人工审核后交接抖音</strong></aside></article>)}</section>}
