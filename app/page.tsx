...
export default function Home() {
  ...
  const [manualEvidenceDraft, setManualEvidenceDraft] = useState<ManualEvidenceDraft>({ leadId:"", sourceName:"", title:"", canonicalUrl:"", publishedAt:"" });
    ...
    <section className="workspace">
      ...
      {view === "ideas" && <>
        ...
        <section className="evidenceGaps">
          <header><div><small>EVIDENCE GAP · HUMAN SHORTLIST</small><b>{evidenceGaps?`${evidenceGaps.summary.leadsReturned} 条补证线索 · ${evidenceGaps.summary.sourcesRepresented} 个来源 · 已选 ${evidenceGapShortlist.length}/3`:"等待单来源线索分析"}</b></div><button type="button" disabled={evidenceGapsBusy} onClick={loadEvidenceGaps}>{evidenceGapsBusy?"分析中…":"生成补证清单（只读）"}</button></header>
          ...
          {evidenceGapShortlist.length>0&&<aside className="evidenceSearchPlan"><div><b>{evidenceSearchPlan?.readyForHumanResearchReview?"检索计划已生成，但尚未执行":`已选 ${evidenceGapShortlist.length} 条，等待生成计划`}</b><span>{evidenceSearchPlan?.readyForHumanResearchReview?`${evidenceSearchPlan.targets.length} 个目标 · ${evidenceSearchPlan.targets.reduce((sum,target)=>sum+target.allowedSources.length,0)} 个允许信源入口 · 已排除 ${evidenceSearchPlan.targets.reduce((sum,target)=>sum+target.independenceDiagnostics.excludedSources.length,0)} 个同源入口`:`最多 3 条 · 只允许公开 RSS 与官方新闻室`}</span>{evidenceSearchPlan?.planFingerprint&&<code>{evidenceSearchPlan.planFingerprint.slice(0,16)}…</code>}</div><div className="evidencePlanActions"><button type="button" disabled={evidenceSearchPlanBusy} onClick={previewEvidenceSearchPlan}>{evidenceSearchPlanBusy?"核对中…":"生成第二来源检索计划（不执行）"}</button><button type="button" disabled={!evidenceSearchPlan?.readyForHumanResearchReview||evidenceMetadataBusy} onClick={previewEvidenceMetadata}>{evidenceMetadataBusy?"检索中…":"检索公开 RSS 元数据"}</button></div></aside>}
          {evidenceSearchPlan?.readyForHumanResearchReview&&<aside className="manualEvidenceForm"><header><b>人工公开来源预览（不保存）</b><span>当前仅支持一次审查 1 个标题；不请求填写的 URL</span></header><div><label>待补证标题<select value={manualEvidenceDraft.leadId} onChange={(event)=>setManualEvidenceDraft((current)=>({...current,leadId:event.target.value}))}><option value="">请选择</option>{evidenceGaps?.leads.filter((lead)=>evidenceGapShortlist.includes(lead.id)).map((lead)=><option key={lead.id} value={lead.id}>{lead.title}</option>)}</select></label><label>来源名称<input value={manualEvidenceDraft.sourceName} maxLength={80} onChange={(event)=>setManualEvidenceDraft((current)=>({...current,sourceName:event.target.value}))}/></label><label>候选标题<input value={manualEvidenceDraft.title} maxLength={300} onChange={(event)=>setManualEvidenceDraft((current)=>({...current,title:event.target.value}))}/></label><label>公开 HTTPS 链接<input type="url" value={manualEvidenceDraft.canonicalUrl} maxLength={2048} onChange={(event)=>setManualEvidenceDraft((current)=>({...current,canonicalUrl:event.target.value}))}/></label><label>发布时间（ISO 8601）<input value={manualEvidenceDraft.publishedAt} maxLength={40} placeholder="2026-08-23T08:00:00Z" onChange={(event)=>setManualEvidenceDraft((current)=>({...current,publishedAt:event.target.value}))}/></label></div><button type="button" disabled={manualEvidenceBusy||!manualEvidenceDraft.leadId||evidenceGapShortlist.length!==1} onClick={previewManualEvidence}>{manualEvidenceBusy?"校验中…":"预览人工来源（不保存）"}</button>{manualEvidencePreview?.readyForHumanEvidenceReview&&manualEvidencePreview.targets?.flatMap((target)=>target.candidates.map((candidate)=><section key={candidate.id}><b>{candidate.title}</b><span>{target.originalHost} → {candidate.candidateHost} · 较原来源 {candidate.publishedDeltaHours>0?"+":""}{candidate.publishedDeltaHours} 小时</span><button type="button" onClick={()=>selectManualEvidenceCandidate(target.leadId,candidate.id)}>进入六项审查预览</button></section>))}{manualEvidencePreview&&<footer>{manualEvidencePreview.readyForHumanEvidenceReview?`已接受 ${manualEvidencePreview.summary?.candidatesAccepted??0} 条临时候选；可进入审查但不能保存来源锁`:`已阻断：${manualEvidencePreview.blockers.join(" / ")}`} · 候选 URL 请求 0 · 保存 0 · 来源锁 0</footer>}</aside>}
