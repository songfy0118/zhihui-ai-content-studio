...
type LocalDraftPreview = { ideaId:string; title:string; body:string; hashtags:string[]; estimatedViews:string; status:"local_preview"|"model_generated_unverified"|"source_locked_review_ready" };
type DouyinConnectorStatus = "idle"|"opening"|"received"|"unavailable";
...
type NewsPreviewStatus = { status:"preview_ready"|"no_live_items";fetchedAt:string;summary:{feedsAttempted:number;readySources:number;failedSources:number;itemsReturned:number};sourceHealth:Array<{sourceId:string;status:"ready"|"empty"|"error";itemsParsed:number;errorCode:string|null;failureClass:string|null;retryable:boolean;operatorAction:string|null}>;transportAssessment:{status:"not_assessed"|"all_feeds_reachable"|"mixed_reachability"|"all_feeds_failed_inconclusive";reachableSources:number;failedSources:number;tlsOrProxyFailures:number;runtimeOutboundReachable:boolean;globalOutageProven:false;operatorAction:string|null};metadataQuality:{asOf:string;itemsAudited:number;provenanceReadyItems:number;usableTimestampItems:number;within24Hours:number;within72Hours:number;within7Days:number;olderThan7Days:number;missingOrInvalidTimestamps:number;futureTimestampsRequiringReview:number;metadataQualityReadyItems:number;articleBodiesFetched:false;factsVerified:false;databaseWrites:false;publishTriggered:false};items:Array<{id:string;sourceName:string;title:string;summary:string;canonicalUrl:string;publishedAt:string|null;ageHours:number|null;freshnessStatus:"timestamp_missing_or_invalid"|"future_timestamp_requires_review"|"within_24_hours"|"within_72_hours"|"within_7_days"|"older_than_7_days";metadataProvenanceReady:boolean}>;contentFetched:boolean;factsVerified:boolean;humanReviewRequired:boolean;externalCalls:number;databaseWrites:boolean;publishTriggered:boolean };
type TopicClusterPreview = { status:"clusters_ready"|"no_items";summary:{itemsConsidered:number;clusterCount:number;crossSourceClusters:number;eligibleCandidates:number;similarityThreshold:number;windowHours:number};inputQualityGate:{required:boolean;itemsReceived:number;itemsAccepted:number;itemsExcluded:number;exclusionReasons:Record<string,number>;articleBodiesFetched:false;factsVerified:false;databaseWrites:false;publishTriggered:false};clusters:Array<{id:string;title:string;status:string;itemCount:number;sourceCount:number;sourceIds:string[];firstSeenAt:string|null;lastSeenAt:string|null;meanSimilarity:number|null;crossSourceConfirmed:boolean;timeWindowVerified:boolean;eligibleForHotspotScoring:boolean}>;factsVerified:boolean;heatScored:boolean;externalCalls:number;databaseWrites:boolean;publishTriggered:boolean };
...
type EvidenceGapPreview = { status:"evidence_gaps_ready"|"no_recent_account_fit_leads";profile:{id:string;label:string;calibration:string};summary:{clustersConsidered:number;recentSingleSourceLeads:number;leadsReturned:number;independentSourcesStillRequired:number;sourcesRepresented:number;languagesRepresented:Array<"zh-CN"|"en">;maxLeadsPerSource:number;windowHours:number;minimumAccountFit:number;leadsWithQualityEvidence:number};qualityBoundary:{clusteringQualityGateRequired:boolean;clusteringItemsReceived:number|null;clusteringItemsAccepted:number|null;clusteringItemsExcluded:number|null;allReturnedLeadsQualityBound:boolean;shortlistQualityFingerprint:string|null;articleBodiesFetched:false;factsVerified:false;databaseWrites:false;publishTriggered:false};leads:Array<{id:string;title:string;category:string;sourceId:string;publishedAt:string;ageHours:number;accountFitScore:number;matchedAccountTopics:string[];status:"needs_independent_source";missingIndependentSources:1;queryLanguage:"zh-CN"|"en"|"und";suggestedQueries:string[];shortlistableForEvidenceSearch:true;factsVerified:false;sourceLockReady:false;selectableForDraft:false;qualityEvidenceFingerprint:string|null;qualityBoundary:{inheritedFromRequiredClusteringGate:boolean;qualityBound:boolean;evidenceItems:number;metadataProvenanceReady:boolean;collectionScope:"rss_metadata_only"|null;articleBodiesFetched:boolean;sourceEvidenceUrls:string[];qualityEvidenceFingerprint:string|null}}>;humanShortlistPersisted:false;evidenceSearchTriggered:false;factsVerified:false;sourceLocksCreated:0;draftsUnlocked:0;externalCalls:number;databaseWrites:false;publishTriggered:false };
type EvidenceSearchPlanPreview = { status:"search_plan_ready"|"search_plan_blocked";readyForHumanResearchReview:boolean;blockers:string[];selection:{requested:number;accepted:number;maximum:number};targets:Array<{leadId:string;title:string;originalSourceId:string;sourcePublishedAt:string;sourceQualityEvidenceFingerprint:string|null;qualityLineageBound:boolean;originalEvidence:Array<{id:string;sourceId:string;sourceName:string;title:string;canonicalUrl:string;publishedAt:string}>;status:"planned_not_executed";queries:string[];allowedSources:Array<{id:string;name:string;sourceType:string;baseUrl:string;feedUrl:string|null;collectionMode:"rss_metadata_only"|"manual_public_page_review";automaticCollectionBlockedReason:string|null}>;allowedSourceSummary:{rssMetadataSources:number;manualPublicPageSources:number};independenceDiagnostics:{policy:"source_id_and_exact_normalized_host";originalHosts:string[];excludedSources:Array<{id:string;name:string;host:string|null;reason:"same_source_id"|"same_exact_host"}>};requiredIndependentSources:number;resultsFound:0;claimsVerified:0;sourceLockReady:false}>;planFingerprint:string|null;qualityBoundTargets:number;allowedMethods:string[];prohibitedMethods:string[];automaticSearchAllowed:false;searchTriggered:false;factsVerified:false;sourceLocksCreated:0;draftsUnlocked:0;databaseWrites:false;publishTriggered:false;externalCalls:number };
type EvidenceCandidateAudit = { targetItemEvaluations:number;excludedBySourceScope:number;excludedByQualityGate:number;qualityExclusionReasons:Record<string,number>;excludedByTimeWindow:number;excludedByTitleMatch:number;matchedBeforeLimit:number;omittedByPerTargetLimit:number;candidatesReturned:number };
type EvidenceMetadataPreview = { status:"metadata_preview_blocked"|"metadata_candidates_found"|"no_metadata_candidates";blockers:string[];planFingerprint:string|null;summary:{targetsReviewed:number;itemsConsidered:number;candidatesReturned:number;candidatesWithQualityEvidence:number};candidateAudit:EvidenceCandidateAudit;qualityBoundary:{required:boolean;sourceTargetsQualityBound:boolean;allReturnedCandidatesQualityBound:boolean;previewQualityFingerprint:string|null;articleBodiesFetched:false;factsVerified:false;databaseWrites:false;publishTriggered:false};targets:Array<{leadId:string;title:string;originalSourceId:string;sourceQualityEvidenceFingerprint:string;originalEvidence:{id:string;sourceId:string;sourceName:string;title:string;canonicalUrl:string;publishedAt:string}|null;originalHost:string|null;candidateCount:number;candidateAudit:EvidenceCandidateAudit;candidates:Array<{id:string;sourceId:string;sourceName:string;title:string;canonicalUrl:string;publishedAt:string;candidateHost:string|null;publishedDeltaHours:number;titleSimilarity:number;sharedTerms:string[];reviewStatus:"human_review_required";candidateQualityEvidenceFingerprint:string;qualityBoundary:{qualityBound:boolean;blocker:string|null;metadataProvenanceReady:boolean;collectionScope:"rss_metadata_only"|null;articleBodiesFetched:boolean;freshnessStatus:string|null;sourceEvidenceUrl:string|null;qualityEvidenceFingerprint:string|null}}>;sourceLockReady:false;factsVerified:false}>;searchScope:string;feedMetadataMatched:boolean;articleBodiesFetched:false;humanReviewRequired?:true;factsVerified:false;sourceLocksCreated:0;draftsUnlocked:0;databaseWrites:false;publishTriggered:false;externalCalls:number };
...
const EMPTY_MANUAL_EVIDENCE_DRAFT:ManualEvidenceDraft = { leadId:"", sourceName:"", publisherRole:"", title:"", canonicalUrl:"", publishedAt:"" };

function syncManualEvidenceDraftToShortlist(current:ManualEvidenceDraft, selectedLeadIds:string[]) {
  const nextLeadId = selectedLeadIds.length === 1 ? selectedLeadIds[0] : "";
  return current.leadId === nextLeadId ? current : { ...EMPTY_MANUAL_EVIDENCE_DRAFT, leadId:nextLeadId };
}
...
type EvidenceReviewPreview = { status:"evidence_review_preview_ready"|"evidence_review_preview_blocked";humanEvidenceReviewComplete:boolean;readyForAuthorizedSourceLockSave:boolean;blockers:string[];downstreamBlockers:string[];manualEvidenceUsed?:boolean;planFingerprint:string|null;reviewFingerprint:string|null;summary:{targetsRequired:number;targetsReviewed:number;targetsEligible:number;targetsWithQualityLineage:number};semanticReview:string;persisted:false;sourceLockCreated:false;factsVerified:false;draftsUnlocked:0;databaseWrites:false;publishTriggered:false;externalCalls:number };
type SourceLockSavePlan = { status:"source_lock_save_plan_ready"|"source_lock_save_plan_blocked";readyForAuthorizationRequest:boolean;blockers:string[];reviewFingerprint:string|null;savePlanFingerprint:string|null;plannedRecordCount:number;plannedLocks:Array<{leadId:string;title:string;reviewFingerprint:string;sources:Array<{evidenceId:string;sourceId:string;sourceName:string;title:string;canonicalUrl:string;publishedAt:string;publisherRole:string;evidenceRole:"original"|"independent"}>;claimCount:0;factsVerified:false;status:"planned_not_saved"}>;authorizationRequired:true;authorizationGranted:false;singleUseAuthorizationRequired:true;writeAllowed:false;persisted:false;sourceLocksCreated:0;factsVerified:false;draftsUnlocked:0;databaseWrites:false;publishTriggered:false;externalCalls:number };
type SourceLockSaveAuthorizationPreview = { status:"source_lock_save_authorization_preview_ready"|"source_lock_save_authorization_preview_blocked";blockers:string[];sourceSavePlanFingerprint:string|null;sourceReviewFingerprint:string|null;authorizationPreviewFingerprint:string|null;requiredConfirmation:string|null;saveTarget:null|{leadId:string;title:string;reviewFingerprint:string;savePlanFingerprint:string;evidenceCount:number;evidenceRoles:string[];operation:string;targetStatus:"preview_only_not_authorized";requiresExactSavePlanFingerprint:true;writeAllowed:false};authorizationTerms:null|{singleUse:true;ttlSecondsAfterAcceptance:300;exactSavePlanFingerprintRequired:true;sourceLockRecordCount:1;evidenceRecordCount:number;allowedOperation:string;liveSaveRouteConnected:boolean};eligibleForExplicitSourceLockSaveAuthorization:boolean;singleUseAuthorizationRequired:true;sourceLockSaveAuthorizationGranted:false;liveSaveRouteConnected:boolean;writeAllowed:false;databaseWriteAttempted:false;databaseWrites:false;persisted:false;sourceLocksCreated:0;draftsUnlocked:0;externalCalls:false;publishTriggered:false;businessResult:false };
type SourceLockStorageReadiness = { mode:"plan_only";localOnly:true;readyToApplyLocally:boolean;blockers:string[];targetBinding:string|null;storageStatus:"missing"|"partial"|"verified"|"unknown";migrationTag:string|null;statementCount:number;onlyCreateStatements:boolean;destructiveStatements:boolean;authorizationRequired:true;applyImplemented:false;applyPerformed:false;databaseWrites:false;externalCalls:false;publishTriggered:false;storage?:{status:"missing"|"partial"|"verified";verified:boolean;missingObjects:string[];missingColumns:string[];expectedObjectCount:number;databaseWrites:false;applyPerformed:false;externalCalls:false};error?:string };
type SourceLockSaveResult = { status:string;eligible?:boolean;blockers:string[];persisted:boolean;alreadyPersisted?:boolean;sourceLockId?:string;sourceLocksCreated:number;databaseWriteAttempted:boolean;databaseWrites:boolean;draftsUnlocked:number;externalCalls:boolean;publishTriggered:boolean };
type UnifiedDraftPackagePlanPreview = { status:"platform_text_unified_draft_package_plan_ready"|"platform_text_unified_draft_package_plan_blocked";blockers:string[];draftPackagePlanFingerprint:string|null;platformCount:number;assetCount:number;copyHandoffReady:boolean;reviewedAssetReferencesReady:boolean;draftPackageInputsReady:boolean;eligibleForCreatorPageOpenAuthorization:boolean;readyForDraftHandoff:boolean;browserOpenPerformed:boolean;loginTriggered:boolean;uploadTriggered:boolean;draftSaved:boolean;databaseWrites:boolean;filesystemMutations:boolean;externalCalls:boolean;publishTriggered:boolean;requestAccepted:boolean;planningOnly:boolean };
type DurableReviewInputReadiness = { status:"platform_text_durable_review_inputs_ready"|"platform_text_durable_review_inputs_blocked";blockers:string[];draftReview:{status:string;found:boolean;reviewedPlatforms:number;durableHumanReview:boolean;durableReviewInputReady:boolean}|null;visualReview:{status:string;found:boolean;reviewedPlatforms:number;reviewedAssets:number;durableHumanReview:boolean;durableVisualReviewInputReady:boolean}|null;durableDraftReviewReady:boolean;durableVisualReviewReady:boolean;inputsReady:boolean;databaseReadAttempted:boolean;databaseReads:number;databaseWrites:false;filesystemMutations:false;externalCalls:false;browserOpenPerformed:false;loginTriggered:false;uploadTriggered:false;draftSaved:false;publishTriggered:false;businessResult:false };
type ReviewReceiptCatalog = { status:"platform_text_review_receipt_catalog_ready"|"platform_text_review_receipt_catalog_blocked";blockers:string[];draftReviews:{fingerprint:string;createdAt:string}[];visualReviews:{fingerprint:string;createdAt:string}[];catalogReadReady:boolean;candidatePairAvailable:boolean;databaseReadAttempted:boolean;databaseReads:number;databaseWrites:false;externalCalls:false;browserOpenPerformed:false;loginTriggered:false;uploadTriggered:false;draftSaved:false;publishTriggered:false };
type ReviewStorageReadiness = { status:"platform_text_review_storage_readiness_ready"|"platform_text_review_storage_readiness_blocked";blockers:string[];draftReviewStorage:{status:"missing"|"partial"|"verified"|"unknown";verified:boolean;missingObjectCount:number;missingColumnCount:number;migrationTag:string}|null;visualReviewStorage:{status:"missing"|"partial"|"verified"|"unknown";verified:boolean;missingObjectCount:number;missingColumnCount:number;migrationTag:string}|null;storageInspectionReady:boolean;bothSchemasVerified:boolean;migrationAuthorizationRequired:boolean;migrationApplyImplemented:false;migrationApplyPerformed:false;databaseReadAttempted:boolean;databaseReads:number;databaseWrites:false;externalCalls:false;publishTriggered:false };
type ReviewMigrationAuthorizationPreview = { status:"platform_text_review_migration_authorization_preview_ready"|"platform_text_review_migration_authorization_preview_blocked";blockers:string[];migrationScopeFingerprint:string|null;requiredConfirmation:string|null;migrationTags:string[];migrationManifest:Array<{tag:string;tables:string[];indexes:string[]}>;tableCount:number;indexCount:number;objectCount:number;localOnly:true;remoteAllowed:false;createOnly:true;eligibleForExplicitLocalMigrationAuthorization:boolean;authorizationRequired:true;authorizationGranted:false;executorConnected:false;commandPrepared:false;applyImplemented:false;applyPerformed:false;databaseWrites:false;externalCalls:false;publishTriggered:false };
type ReviewMigrationIsolatedRehearsal = { status:"platform_text_review_migration_isolated_rehearsal_verified"|"platform_text_review_migration_isolated_rehearsal_failed"|"platform_text_review_migration_isolated_rehearsal_blocked";blockers:string[];mode:"isolated_in_memory_sqlite";migrationTags:string[];appliedTags:string[];tableCount:number;indexCount:number;schemaVerified:boolean;successPathVerified:boolean;rollbackScenarios:Array<{tag:string;rollbackPerformed:boolean;rollbackVerified:boolean}>;rollbackScenarioCount:number;rollbackVerifiedCount:number;failurePathVerified:boolean;intentionalFailureProbes:number;ephemeralDatabaseWrites:boolean;liveDatabaseAccessed:false;liveDatabaseWrites:false;liveApplyPerformed:false;externalCalls:false;publishTriggered:false;businessResult:false };
type ReviewMigrationExecutionPreflight = { status:"platform_text_review_migration_execution_preflight_ready"|"platform_text_review_migration_execution_preflight_blocked";blockers:string[];sourceMigrationScopeFingerprint:string|null;migrationExecutionPreflightFingerprint:string|null;migrationTags:string[];requiredConfirmation:string|null;evidenceValidated:boolean;ephemeralEvidenceUsed:boolean;readyForExplicitAuthorizationRequest:boolean;singleUseAuthorizationRequired:true;confirmationReceived:false;authorizationGranted:false;authorizationConsumed:false;executionContract:null;executorConnected:false;applyImplemented:false;applyPerformed:false;databaseReadAttempted:false;databaseReads:0;databaseWrites:false;externalCalls:false;publishTriggered:false;businessResult:false };
type ReviewMigrationLocalTargetDiagnostic = { status:"platform_text_review_migration_local_target_diagnostic_verified"|"platform_text_review_migration_local_target_diagnostic_blocked";blockers:string[];migrationTags:string[];targetBinding:string|null;bindingScope:"loopback_miniflare_only"|"unverified";databaseNameConfigured:boolean;placeholderLocalDatabaseId:boolean;loopbackDevConfig:boolean;migrationJournalVerified:boolean;localStateCandidateCount:number;localTargetEvidenceFingerprint:string|null;localTargetProven:boolean;readyForExplicitAuthorizationRequest:boolean;confirmationReceived:false;authorizationGranted:false;authorizationConsumed:false;commandPrepared:false;executorConnected:false;databaseFileOpened:false;databaseReadAttempted:false;databaseReads:0;databaseWrites:false;filesystemWrites:false;externalCalls:false;publishTriggered:false;businessResult:false };
type ReviewMigrationSingleUseAuthorizationRequest = { status:"platform_text_review_migration_single_use_authorization_request_ready"|"platform_text_review_migration_single_use_authorization_request_blocked";blockers:string[];migrationTags:string[];sourceMigrationScopeFingerprint:string|null;migrationExecutionPreflightFingerprint:string|null;localTargetEvidenceFingerprint:string|null;authorizationRequestFingerprint:string|null;requiredConfirmation:string|null;ticketTerms:{singleUse:true;ttlSecondsAfterAcceptance:300;localTargetOnly:true;remoteAllowed:false;exactFingerprintsRequired:true;createOnlyMigrations:true;rollbackOnFailureRequired:true;postApplyReadOnlyVerificationRequired:true};evidenceValidated:boolean;readyForHumanConfirmation:boolean;confirmationReceived:false;authorizationGranted:false;authorizationConsumed:false;authorizationTicket:null;executionContract:null;commandPrepared:false;executorConnected:false;applyImplemented:false;applyPerformed:false;databaseFileOpened:false;databaseReadAttempted:false;databaseReads:0;databaseWrites:false;filesystemWrites:false;externalCalls:false;publishTriggered:false;businessResult:false };
type ReviewMigrationPostApplyVerificationPlan = { status:"platform_text_review_migration_post_apply_verification_plan_ready"|"platform_text_review_migration_post_apply_verification_plan_blocked";blockers:string[];migrationTags:string[];sourceAuthorizationRequestFingerprint:string|null;verificationPlanFingerprint:string|null;expectedCounts:{tables:5;indexes:12;columns:34;metadataQueries:7};plannedChecks:string[];verificationMode:"post_apply_read_only_metadata";rollbackProofExpectation:{required:true;availableBeforeApply:false;status:"awaiting_authorized_transaction";failureInjectionForbiddenOnLiveTarget:true;rollbackEvidenceRequiredOnFailure:true};evidenceValidated:boolean;readyForFuturePostApplyVerification:boolean;preApplySnapshotCaptured:false;confirmationReceived:false;authorizationGranted:false;authorizationTicket:null;executionContract:null;commandPrepared:false;applyPerformed:false;verificationPerformed:false;rollbackPerformed:false;databaseReadAttempted:false;databaseReads:0;inspectedBusinessRows:false;databaseWrites:false;filesystemWrites:false;externalCalls:false;publishTriggered:false;businessResult:false };
...
type LocalRuntimeStatus = { status:"loading"|"current"|"services_offline"|"bridge_stale"|"models_missing"|"blocked";current:boolean;services:Array<{id:string;online:boolean;statusCode:number|null;modelReady?:boolean|null}>;offlineServices:string[];nextAction:string;processMutation:boolean;externalCalls:boolean;modelCalls:boolean;downloads:boolean;costIncurred:boolean;publishTriggered:boolean };
...
function ManualEvidenceReviewHandoff({ decisions, preview }:{ decisions:Record<string,EvidenceReviewDecision>;preview:EvidenceReviewPreview|null }) {
  const manualDecision = Object.values(decisions).find((decision)=>decision.candidateMode==="manual_public_metadata");
  if (!manualDecision) return null;
  const confirmedChecks = Object.values(manualDecision.checks).filter(Boolean).length;
  return <aside className={preview?.readyForAuthorizedSourceLockSave?"evidenceReviewHandoff ready":"evidenceReviewHandoff"} role="status" aria-live="polite"><small>MANUAL EVIDENCE → REVIEW</small><b>{describeManualEvidenceReviewHandoff(confirmedChecks,evidenceReviewChecklist.length,preview?.readyForAuthorizedSourceLockSave===true)}</b><span>当前候选和勾选仍只是本页临时状态；“已交接”不代表事实已核验，也不会创建来源锁。</span><code>人工判断 {confirmedChecks}/{evidenceReviewChecklist.length} · 保存 0 · 来源锁 0</code></aside>;
}
...
function SourceLockPlanSummary({ plan }:{ plan:SourceLockSavePlan }) {
  if (!plan.readyForAuthorizationRequest || !plan.plannedLocks.length) return null;
  return <section className="sourceLockPlanSummary" aria-label="授权前来源锁计划摘要"><header><small>SOURCE LOCK PLAN · PRE-AUTHORIZATION</small><b>授权前请核对两条公开来源</b><span>这里只展示计划，不代表已保存或已核验事实</span></header>{plan.plannedLocks.map((lock)=><article key={lock.leadId}><strong>{lock.title}</strong><div>{lock.sources.map((source)=><a key={`${lock.leadId}:${source.evidenceRole}`} href={source.canonicalUrl} target="_blank" rel="noreferrer"><small>{formatSourceLockEvidenceRole(source.evidenceRole)} · {formatSourceLockPublisherRole(source.publisherRole)}</small><b>{source.sourceName}</b><span>{source.title}</span><time>{source.publishedAt}</time></a>)}</div></article>)}<footer>审查指纹 {plan.reviewFingerprint?.slice(0,16)??"无"} · 计划指纹 {plan.savePlanFingerprint?.slice(0,16)??"无"} · 事实核验 0 · 保存 0 · 来源锁 0</footer></section>;
}
function SourceLockAuthorizationTerms({ preview }:{ preview:SourceLockSaveAuthorizationPreview }) {
  ...
  return <section className="sourceLockAuthorizationTerms" aria-label="来源锁单次授权预览条款"><header><small>SINGLE-USE AUTHORIZATION · PREVIEW ONLY</small><b>授权范围已锁定，但尚未授权</b><span>本人精确确认后，只允许写入当前计划中的记录</span></header><div><span><b>{preview.authorizationTerms.sourceLockRecordCount}</b> 条来源锁</span><span><b>{preview.authorizationTerms.evidenceRecordCount}</b> 条证据记录</span><span><b>1</b> 次使用</span><span><b>{preview.authorizationTerms.ttlSecondsAfterAcceptance/60}</b> 分钟有效</span></div><p>必须精确匹配计划指纹 <code>{preview.sourceSavePlanFingerprint?.slice(0,16)}…</code>；未来授权票据从接受时开始计时，过期或重放均应失效。</p>{preview.requiredConfirmation&&<code>{preview.requiredConfirmation}</code>}<footer>本机保存路由已连接 · 下方需本人精确确认 · 当前授权 0 · 写库 0 · 来源锁 0</footer></section>;
}
function SourceLockStorageReadinessCard({ readiness, busy, onInspect }:{ readiness:SourceLockStorageReadiness|null;busy:boolean;onInspect:()=>void }) {
  ...
  const headline = storage?.status==="verified"?"来源锁表结构已就绪":storage?.status==="missing"?"来源锁表尚未创建":storage?.status==="partial"?"来源锁表结构不完整":readiness?"存储诊断不可用":"保存前先检查本机表结构";
  return <section className={storage?.verified?"sourceLockStorageReadiness ready":"sourceLockStorageReadiness"}><header><div><small>SOURCE LOCK STORAGE · READ ONLY</small><b>{headline}</b><span>{storage?.verified?"可继续请求精确保存授权；本次检查没有写入":storage?.status==="missing"?`仅检测到可回滚的创建计划 ${readiness?.migrationTag??"0007"}，尚未执行`:storage?.status==="partial"?"部分对象或字段存在，禁止自动迁移":"只读取 sqlite_schema 与表字段，不执行迁移"}</span></div><button type="button" disabled={busy} onClick={onInspect}>{busy?"检查中…":"检查来源锁表（只读）"}</button></header>{readiness&&<div><span><b>{storage?.status??"不可用"}</b>存储状态</span><span><b>{storage?.missingObjects.length??0}</b>缺失对象</span><span><b>{storage?.missingColumns.length??0}</b>缺失字段</span><span><b>{readiness.statementCount??0}</b>计划语句</span></div>}{readiness?.blockers.length?<p>当前阻塞：{readiness.blockers.join(" · ")}</p>:null}<footer>结构查询只读 · 授权 0 · 迁移执行 0 · 数据库写入 0 · 来源锁 0</footer></section>;
}
function SourceLockSaveBoundarySummary({ plan, preview, readiness }:{ plan:SourceLockSavePlan;preview:SourceLockSaveAuthorizationPreview|null;readiness:SourceLockStorageReadiness|null }) {
  const gates = [
...
const fallbackIdeas: Idea[] = [
  ...
  ["ai-search","AI 搜索会取代传统搜索引擎吗？","比较答案质量、广告模式、来源透明度与用户习惯，而不是只看单次演示","AI",91,94,93],
  ["stablecoin-payments","稳定币正在成为跨境支付的新基础设施吗？","从结算速度、监管、成本和真实机构采用情况拆解","金融科技",88,92,90],
  ["ai-datacenter-power","AI 数据中心为什么开始争夺电力？","追踪官方资本开支、电网容量与能耗披露，区分规划和已投产项目","科技金融",93,91,92],
  ["small-models","小模型会成为企业 AI 的真正主力吗？","比较部署成本、隐私、延迟和任务准确率，不把参数规模当成唯一标准","企业AI",90,92,94],
  ["robotaxi-scale","Robotaxi 从试运营到规模化还差什么？","核对运营区域、安全员要求、车队规模和监管条件","机器人",89,95,91],
  ["ai-copyright","生成式 AI 的版权规则正在变清楚吗？","从法院文件、授权协议和平台政策分别梳理，不把争议写成定论","AI政策",87,93,95],
  ["quantum-commercial","量子计算离商业回报还有多远？","区分科研里程碑、硬件路线和企业合同，避免用实验结果替代商业收入","前沿科技",86,92,90],
...
const buildDemoViewRange = (score:number) => {
  const low = Math.max(1000, Math.round(((score - 78) * 420) / 500) * 500);
  const high = Math.round((low * 1.9) / 500) * 500;
  return `${low.toLocaleString("zh-CN")}–${high.toLocaleString("zh-CN")}`;
};
const buildLocalDraftPreview = (idea:Idea):LocalDraftPreview => ({
  ideaId:idea.id,
  title:idea.title.replace(/[？?]$/u, ""),
  body:`先说结论：${idea.angle}。\n\n这是一份本地雏形草稿，正式版本必须补齐来源、事实核验和账号风格后才能进入人工审核。`,
  hashtags:[idea.category.replace(/\s+/gu, ""), "AI科技", "热点观察"],
  estimatedViews:buildDemoViewRange(idea.douyinScore),
...
const unifiedDraftPackageBlockerLabels: Record<string,string> = {
  platform_text_visual_asset_plan_invalid_or_stale:"双平台文案交接或视觉资产计划缺失 / 已过期",
  platform_text_svg_render_invalid_or_stale:"SVG 卡片渲染与完整性检查尚未完成",
  platform_text_visual_asset_handoff_invalid_or_stale:"持久化人工视觉审核与素材交接尚未完成",
  platform_text_draft_handoff_plan_invalid_or_stale:"小红书与抖音文案交接计划缺失 / 已过期",
};
const durableReviewInputBlockerLabels: Record<string,string> = {
  platform_text_draft_review_fingerprint_invalid:"文案审核指纹必须是 64 位小写哈希",
  platform_text_visual_review_fingerprint_invalid:"视觉审核指纹必须是 64 位小写哈希",
  platform_text_draft_review_not_found:"D1 中没有找到对应的文案审核记录",
  platform_text_visual_review_not_found:"D1 中没有找到对应的视觉审核记录",
  platform_text_durable_review_storage_unavailable:"D1 审核存储尚未连接或迁移未就绪",
};
const reviewReceiptCatalogBlockerLabels: Record<string,string> = {
  platform_text_review_receipt_catalog_storage_unavailable:"D1 审核存储尚未连接或迁移未就绪",
  platform_text_review_receipt_catalog_query_failed:"D1 审核收据表不可读或迁移未完成",
  platform_text_draft_review_catalog_invalid:"文案审核收据目录数据无效",
  platform_text_visual_review_catalog_invalid:"视觉审核收据目录数据无效",
};
...
export default function Home() {
  ...
  const [localDraftPreviews, setLocalDraftPreviews] = useState<LocalDraftPreview[]>([]);
  const localDraftPreviewRef = useRef<HTMLElement | null>(null);
  const [douyinConnectorStatus, setDouyinConnectorStatus] = useState<DouyinConnectorStatus>("idle");
  ...
  const [manualEvidenceDraft, setManualEvidenceDraft] = useState<ManualEvidenceDraft>({ ...EMPTY_MANUAL_EVIDENCE_DRAFT });
  ...
  const [sourceLockStorageReadiness, setSourceLockStorageReadiness] = useState<SourceLockStorageReadiness|null>(null);
  const [sourceLockStorageReadinessBusy, setSourceLockStorageReadinessBusy] = useState(false);
  const [sourceLockSaveConfirmation, setSourceLockSaveConfirmation] = useState("");
  const [sourceLockSaveBusy, setSourceLockSaveBusy] = useState(false);
  const [sourceLockSaveResult, setSourceLockSaveResult] = useState<SourceLockSaveResult|null>(null);
  const [unifiedDraftPackagePlan, setUnifiedDraftPackagePlan] = useState<UnifiedDraftPackagePlanPreview|null>(null);
  const [unifiedDraftPackagePlanBusy, setUnifiedDraftPackagePlanBusy] = useState(false);
  const [draftReviewFingerprint, setDraftReviewFingerprint] = useState("");
  const [visualReviewFingerprint, setVisualReviewFingerprint] = useState("");
  const [durableReviewInputs, setDurableReviewInputs] = useState<DurableReviewInputReadiness|null>(null);
  const [durableReviewInputsBusy, setDurableReviewInputsBusy] = useState(false);
  const [reviewReceiptCatalog, setReviewReceiptCatalog] = useState<ReviewReceiptCatalog|null>(null);
  const [reviewReceiptCatalogBusy, setReviewReceiptCatalogBusy] = useState(false);
  const [reviewStorageReadiness, setReviewStorageReadiness] = useState<ReviewStorageReadiness|null>(null);
  const [reviewStorageReadinessBusy, setReviewStorageReadinessBusy] = useState(false);
  const [reviewMigrationAuthorizationPreview, setReviewMigrationAuthorizationPreview] = useState<ReviewMigrationAuthorizationPreview|null>(null);
  const [reviewMigrationAuthorizationPreviewBusy, setReviewMigrationAuthorizationPreviewBusy] = useState(false);
  const [reviewMigrationIsolatedRehearsal, setReviewMigrationIsolatedRehearsal] = useState<ReviewMigrationIsolatedRehearsal|null>(null);
  const [reviewMigrationIsolatedRehearsalBusy, setReviewMigrationIsolatedRehearsalBusy] = useState(false);
  const [reviewMigrationExecutionPreflight, setReviewMigrationExecutionPreflight] = useState<ReviewMigrationExecutionPreflight|null>(null);
  const [reviewMigrationExecutionPreflightBusy, setReviewMigrationExecutionPreflightBusy] = useState(false);
  const [reviewMigrationLocalTargetDiagnostic, setReviewMigrationLocalTargetDiagnostic] = useState<ReviewMigrationLocalTargetDiagnostic|null>(null);
  const [reviewMigrationLocalTargetDiagnosticBusy, setReviewMigrationLocalTargetDiagnosticBusy] = useState(false);
  const [reviewMigrationSingleUseAuthorizationRequest, setReviewMigrationSingleUseAuthorizationRequest] = useState<ReviewMigrationSingleUseAuthorizationRequest|null>(null);
  const [reviewMigrationSingleUseAuthorizationRequestBusy, setReviewMigrationSingleUseAuthorizationRequestBusy] = useState(false);
  const [reviewMigrationPostApplyVerificationPlan, setReviewMigrationPostApplyVerificationPlan] = useState<ReviewMigrationPostApplyVerificationPlan|null>(null);
  const [reviewMigrationPostApplyVerificationPlanBusy, setReviewMigrationPostApplyVerificationPlanBusy] = useState(false);
  ...
  useEffect(() => {
    if (view !== "production" || localDraftPreviews.length === 0) return;
    localDraftPreviewRef.current?.scrollIntoView({ behavior:"smooth", block:"start" });
  }, [localDraftPreviews, view]);
  ...
  useEffect(() => {
    const receiveConnectorStatus = (event:MessageEvent) => {
      if (event.source !== window || event.data?.type !== "ZHIHUI_DOUYIN_CONNECTOR_ACK_V1") return;
      setDouyinConnectorStatus("received");
    };
    window.addEventListener("message",receiveConnectorStatus);
    return () => window.removeEventListener("message",receiveConnectorStatus);
  }, []);

  const clearSourceLockSavePreviews = () => {
    setSourceLockSavePlan(null);
    setSourceLockSaveAuthorizationPreview(null);
    setSourceLockStorageReadiness(null);
    setSourceLockSaveConfirmation("");
    setSourceLockSaveResult(null);
  ...
  const loadTopicRanking = async () => {
    ...
    try {
      ...
      if (payload.evidenceGapFallback) {
        ...
        setManualEvidenceDraft({ ...EMPTY_MANUAL_EVIDENCE_DRAFT });
  ...
  const toggleEvidenceGap = (id:string) => {
    ...
    setEvidenceReviewPreview(null);
    clearSourceLockSavePreviews();
    const nextShortlist = evidenceGapShortlist.includes(id) ? evidenceGapShortlist.filter((candidateId) => candidateId !== id) : evidenceGapShortlist.length < 3 ? [...evidenceGapShortlist, id] : evidenceGapShortlist;
    setEvidenceGapShortlist(nextShortlist);
    setManualEvidenceDraft((current) => syncManualEvidenceDraftToShortlist(current, nextShortlist));
  };
  ...
  const previewManualEvidence = async () => {
    if (manualEvidenceTextAssessment.blocksPreview) {
      setMessage(`${manualEvidenceTextAssessment.sourceName.message ?? manualEvidenceTextAssessment.title.message}；未发送预览请求。`);
      return;
    }
    if (manualSourceLinkHostAssessment.blocksPreview) {
      setMessage(`${manualSourceLinkHostAssessment.message}；未发送预览请求。`);
      return;
    }
    if (manualEvidencePublishedAtAssessment.blocksPreview) {
      setMessage(`${manualEvidencePublishedAtAssessment.message}；未发送预览请求。`);
      return;
    }
    if (!manualEvidenceFormReadiness.ready || evidenceGapShortlist.length !== 1) {
      setMessage(`人工补证尚未完整：${manualEvidenceFormReadiness.missingLabels.join(" / ") || "请选择且仅选择 1 条线索"}；未发送预览请求。`);
      return;
    }
  ...
  const inspectSourceLockStorageReadiness = async () => {
    setSourceLockStorageReadinessBusy(true);
    try {
      const response = await fetch("/api/local/source-lock-migration", { cache:"no-store" });
      setSourceLockStorageReadiness(await response.json() as SourceLockStorageReadiness);
    } catch {
      setMessage("来源锁表结构只读检查失败；没有执行迁移、写入数据库或创建来源锁。");
    } finally {
      setSourceLockStorageReadinessBusy(false);
    }
  };
  const saveReviewedSourceLock = async () => {
    const requiredConfirmation = sourceLockSaveAuthorizationPreview?.requiredConfirmation;
    const savePlanFingerprint = sourceLockSavePlan?.savePlanFingerprint;
    if (!sourceLockSavePlan?.readyForAuthorizationRequest || !sourceLockSaveAuthorizationPreview?.eligibleForExplicitSourceLockSaveAuthorization || !sourceLockStorageReadiness?.storage?.verified || !requiredConfirmation || sourceLockSaveConfirmation !== requiredConfirmation || !savePlanFingerprint) return;
    setSourceLockSaveBusy(true);
    setSourceLockSaveResult(null);
    try {
      const response = await fetch("/api/local/source-lock-save", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ plan:sourceLockSavePlan, executeRequested:true, confirmation:sourceLockSaveConfirmation, authorizedSavePlanFingerprint:savePlanFingerprint }) });
      const result = await response.json() as SourceLockSaveResult;
      setSourceLockSaveResult(result);
      setMessage(result.persisted || result.alreadyPersisted ? "已保存当前人工复核的来源锁；这不代表事实已自动核验，也不会自动生成或发布草稿。" : `来源锁未保存：${result.blockers.join(" · ") || "保存请求被阻断"}`);
    } catch {
      setMessage("来源锁保存失败；没有解锁草稿、打开平台或发布内容。");
    } finally {
      setSourceLockSaveBusy(false);
    }
  };
  const previewUnifiedDraftPackagePlan = async () => {
    setUnifiedDraftPackagePlanBusy(true);
    setUnifiedDraftPackagePlan(null);
    try {
      const response = await fetch("/api/news/platform-text-unified-draft-package-plan", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({}) });
      setUnifiedDraftPackagePlan(await response.json() as UnifiedDraftPackagePlanPreview);
    } catch {
      setMessage("双平台统一草稿包输入检查失败；没有打开浏览器、登录、上传、保存草稿或发布。");
    } finally {
      setUnifiedDraftPackagePlanBusy(false);
    }
  };
  const checkDurableReviewInputs = async () => {
    setDurableReviewInputsBusy(true);
    setDurableReviewInputs(null);
    try {
      const response = await fetch("/api/news/platform-text-durable-review-input-readiness", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ draftReviewFingerprint, visualReviewFingerprint }) });
      setDurableReviewInputs(await response.json() as DurableReviewInputReadiness);
    } catch {
      setMessage("持久化审核输入查询失败；没有写入数据库、打开浏览器、登录、上传、保存草稿或发布。");
    } finally {
      setDurableReviewInputsBusy(false);
    }
  };
  const discoverReviewReceipts = async () => {
    setReviewReceiptCatalogBusy(true);
    setReviewReceiptCatalog(null);
    try {
      const response = await fetch("/api/news/platform-text-review-receipt-catalog");
      setReviewReceiptCatalog(await response.json() as ReviewReceiptCatalog);
    } catch {
      setMessage("审核收据目录读取失败；没有写入数据库、打开浏览器、登录、上传、保存草稿或发布。");
    } finally {
      setReviewReceiptCatalogBusy(false);
    }
  };
  const inspectReviewStorage = async () => {
    setReviewStorageReadinessBusy(true);
    setReviewStorageReadiness(null);
    setReviewMigrationAuthorizationPreview(null);
    setReviewMigrationIsolatedRehearsal(null);
    setReviewMigrationExecutionPreflight(null);
    setReviewMigrationLocalTargetDiagnostic(null);
    setReviewMigrationSingleUseAuthorizationRequest(null);
    setReviewMigrationPostApplyVerificationPlan(null);
    try {
      const response = await fetch("/api/news/platform-text-review-storage-readiness");
      setReviewStorageReadiness(await response.json() as ReviewStorageReadiness);
    } catch {
      setMessage("审核表结构诊断失败；没有应用迁移或写入数据库。");
    } finally {
      setReviewStorageReadinessBusy(false);
    }
  };
  const previewReviewMigrationAuthorization = async () => {
    if (!reviewStorageReadiness) return;
    setReviewMigrationAuthorizationPreviewBusy(true);
    setReviewMigrationAuthorizationPreview(null);
    setReviewMigrationExecutionPreflight(null);
    setReviewMigrationLocalTargetDiagnostic(null);
    setReviewMigrationSingleUseAuthorizationRequest(null);
    setReviewMigrationPostApplyVerificationPlan(null);
    try {
      const response = await fetch("/api/news/platform-text-review-migration-authorization-preview", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ storageReadiness:reviewStorageReadiness }) });
      setReviewMigrationAuthorizationPreview(await response.json() as ReviewMigrationAuthorizationPreview);
    } catch {
      setMessage("迁移授权范围预览失败；没有准备命令、应用迁移或写入数据库。");
    } finally {
      setReviewMigrationAuthorizationPreviewBusy(false);
    }
  };
  const rehearseReviewMigrationsInMemory = async () => {
    setReviewMigrationIsolatedRehearsalBusy(true);
    setReviewMigrationIsolatedRehearsal(null);
    setReviewMigrationExecutionPreflight(null);
    setReviewMigrationLocalTargetDiagnostic(null);
    setReviewMigrationSingleUseAuthorizationRequest(null);
    setReviewMigrationPostApplyVerificationPlan(null);
    try {
      const response = await fetch("/api/news/platform-text-review-migration-isolated-rehearsal", { method:"POST" });
      setReviewMigrationIsolatedRehearsal(await response.json() as ReviewMigrationIsolatedRehearsal);
    } catch {
      setMessage("审核存储迁移隔离演练失败；没有访问真实 D1、应用正式迁移或写入业务数据。");
    } finally {
      setReviewMigrationIsolatedRehearsalBusy(false);
    }
  };
  const previewReviewMigrationExecutionPreflight = async () => {
    if (!reviewMigrationAuthorizationPreview || !reviewMigrationIsolatedRehearsal) return;
    setReviewMigrationExecutionPreflightBusy(true);
    setReviewMigrationExecutionPreflight(null);
    setReviewMigrationLocalTargetDiagnostic(null);
    setReviewMigrationSingleUseAuthorizationRequest(null);
    setReviewMigrationPostApplyVerificationPlan(null);
    try {
      const response = await fetch("/api/news/platform-text-review-migration-execution-preflight", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ authorizationPreview:reviewMigrationAuthorizationPreview, isolatedRehearsal:reviewMigrationIsolatedRehearsal }) });
      setReviewMigrationExecutionPreflight(await response.json() as ReviewMigrationExecutionPreflight);
    } catch {
      setMessage("迁移执行前检查失败；没有收到授权、连接执行器、应用迁移或写入数据库。");
    } finally {
      setReviewMigrationExecutionPreflightBusy(false);
    }
  };
  const diagnoseReviewMigrationLocalTarget = async () => {
    if (!reviewMigrationExecutionPreflight) return;
    setReviewMigrationLocalTargetDiagnosticBusy(true);
    setReviewMigrationLocalTargetDiagnostic(null);
    setReviewMigrationSingleUseAuthorizationRequest(null);
    setReviewMigrationPostApplyVerificationPlan(null);
    try {
      const response = await fetch("/api/news/platform-text-review-migration-local-target-diagnostic", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ executionPreflight:reviewMigrationExecutionPreflight }) });
      setReviewMigrationLocalTargetDiagnostic(await response.json() as ReviewMigrationLocalTargetDiagnostic);
    } catch {
      setMessage("本机 D1 目标诊断失败；没有准备命令、打开数据库、应用迁移或写入数据。");
    } finally {
      setReviewMigrationLocalTargetDiagnosticBusy(false);
    }
  };
  const previewReviewMigrationSingleUseAuthorizationRequest = async () => {
    if (!reviewMigrationExecutionPreflight || !reviewMigrationLocalTargetDiagnostic) return;
    setReviewMigrationSingleUseAuthorizationRequestBusy(true);
    setReviewMigrationSingleUseAuthorizationRequest(null);
    setReviewMigrationPostApplyVerificationPlan(null);
    try {
      const response = await fetch("/api/news/platform-text-review-migration-single-use-authorization-request", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ executionPreflight:reviewMigrationExecutionPreflight, localTargetDiagnostic:reviewMigrationLocalTargetDiagnostic }) });
      setReviewMigrationSingleUseAuthorizationRequest(await response.json() as ReviewMigrationSingleUseAuthorizationRequest);
    } catch {
      setMessage("单次授权请求预览失败；没有接收确认、签发票据、准备命令、应用迁移或写入数据库。");
    } finally {
      setReviewMigrationSingleUseAuthorizationRequestBusy(false);
    }
  };
  const previewReviewMigrationPostApplyVerificationPlan = async () => {
    if (!reviewMigrationSingleUseAuthorizationRequest) return;
    setReviewMigrationPostApplyVerificationPlanBusy(true);
    setReviewMigrationPostApplyVerificationPlan(null);
    try {
      const response = await fetch("/api/news/platform-text-review-migration-post-apply-verification-plan", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ authorizationRequest:reviewMigrationSingleUseAuthorizationRequest }) });
      setReviewMigrationPostApplyVerificationPlan(await response.json() as ReviewMigrationPostApplyVerificationPlan);
    } catch {
      setMessage("应用后只读校验计划生成失败；没有接收授权、执行 SQL、读取或写入数据库。");
    } finally {
      setReviewMigrationPostApplyVerificationPlanBusy(false);
    }
  };
  useEffect(() => {
  ...
  const ideaPageCount = Math.max(1, Math.ceil(ideas.length / TOPICS_PER_PAGE));
  const visibleIdeas = ideas.slice((ideaPage - 1) * TOPICS_PER_PAGE, ideaPage * TOPICS_PER_PAGE);
  ...
  const queueGeneration = async () => {
    ...
    if (platforms.includes("douyin")) {
      void Promise.all(selected.map(async (idea) => {
        const response = await fetch("/api/local/topic-draft", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(idea) });
        const result = await response.json();
        if (!response.ok || result.status !== "model_generated_unverified") throw new Error(result.blockers?.join(" · ") || "本机中文模型生成失败");
        return { ...result.draft, estimatedViews:buildDemoViewRange(idea.douyinScore) } as LocalDraftPreview;
      })).then((previews) => {
        setLocalDraftPreviews(previews);
        setMessage(`本机 qwen3:4b 已生成 ${previews.length} 份中文草稿雏形；尚未补齐来源，不能直接发布。`);
      }).catch(() => {
        setMessage("已显示规则草稿雏形；本机中文模型暂未返回，来源锁和发布仍保持关闭。");
      });
    }
  ...
  const handoffLocalDraftToDouyin = async (draft:LocalDraftPreview) => {
    if (draft.status !== "source_locked_review_ready") {
      setDouyinConnectorStatus("idle");
      setMessage("这份草稿还没有绑定并人工核验公开来源，暂不能填入抖音。");
      return;
    }
    ...
    const payload = {
      ...
      summary:draft.body.replace(/\s+/gu," ").slice(0,30),
      body:`${draft.body}\n\n${draft.hashtags.map((tag)=>`#${tag}`).join(" ")}`,
      createdAt:new Date().toISOString(),
      saveAllowed:false,
      publishAllowed:false,
    };
    setDouyinConnectorStatus("opening");
    localStorage.setItem("zhihui:douyin-handoff:v1",JSON.stringify(payload));
    window.postMessage({type:"ZHIHUI_DOUYIN_HANDOFF_V1",payload},window.location.origin);
    try { await navigator.clipboard.writeText(`${payload.title}\n\n${payload.body}`); } catch { /* connector remains the primary handoff */ }
    window.open(creatorUrl,"_blank","noopener,noreferrer");
    window.setTimeout(()=>setDouyinConnectorStatus((status)=>status==="received"?status:"unavailable"),1200);
  };
    ...
    <section className="workspace">
      ...
      {view === "ideas" && <>
        ...
        <section className="newsPreview">
          ...
          {newsPreview?.items.length ? <div>{newsPreview.items.slice(0,6).map((item)=><a href={item.canonicalUrl} target="_blank" rel="noreferrer" key={item.id}><small>{item.sourceName} · {item.publishedAt?new Date(item.publishedAt).toLocaleString("zh-CN"):"时间未提供"} · {rssFreshnessLabels[item.freshnessStatus]} · {item.metadataProvenanceReady?"登记来源":"来源待复核"}</small><b>{item.title}</b>{item.summary&&<span>{item.summary}</span>}</a>)}</div> : <p>{newsPreview?"本次没有解析出可展示的真实条目；查看信源健康状态后再处理。":"点击后只读取已登记的公开 RSS；不会写数据库，不会把标题自动当成已核验事实。"}</p>}
          ...
          {newsPreview&&<section className="newsMetadataQuality"><header><b>RSS 元数据质量审计</b><em>可进入时间窗判断 {newsPreview.metadataQuality.metadataQualityReadyItems}/{newsPreview.metadataQuality.itemsAudited}</em></header><div><span><b>{newsPreview.metadataQuality.provenanceReadyItems}</b>登记来源证明</span><span><b>{newsPreview.metadataQuality.within24Hours}</b>24 小时内</span><span><b>{newsPreview.metadataQuality.within72Hours}</b>72 小时内</span><span><b>{newsPreview.metadataQuality.within7Days}</b>7 天内</span><span><b>{newsPreview.metadataQuality.missingOrInvalidTimestamps+newsPreview.metadataQuality.futureTimestampsRequiringReview}</b>时间待复核</span></div><footer>文章正文读取 0 · 事实核验 0 · 数据库写入 0 · 发布 0</footer></section>}
        ...
        <section className="topicClusters">
          ...
          <footer>{topicClusters?`质量门 ${topicClusters.inputQualityGate.itemsAccepted}/${topicClusters.inputQualityGate.itemsReceived} · 聚类前拦截 ${topicClusters.inputQualityGate.itemsExcluded}`:"质量门等待 RSS 元数据"} · 事实核验 {topicClusters?.factsVerified?"已完成":"未完成"} · 热度评分 {topicClusters?.heatScored?"已执行":"0"} · 外部请求 {topicClusters?.externalCalls??0} · 写库 {topicClusters?.databaseWrites?"已发生":"0"} · 发布 {topicClusters?.publishTriggered?"已触发":"0"}</footer>
        </section>
        ...
        <section className="evidenceGaps">
          <header><div><small>EVIDENCE GAP · HUMAN SHORTLIST</small><b>{evidenceGaps?`${evidenceGaps.summary.leadsReturned} 条补证线索 · 质量链 ${evidenceGaps.summary.leadsWithQualityEvidence}/${evidenceGaps.summary.leadsReturned} · ${evidenceGaps.summary.sourcesRepresented} 个来源 · 语言 ${evidenceGaps.summary.languagesRepresented.join(" / ")||"未识别"} · 已选 ${evidenceGapShortlist.length}/3`:"等待单来源线索分析"}</b></div><button type="button" disabled={evidenceGapsBusy} onClick={loadEvidenceGaps}>{evidenceGapsBusy?"分析中…":"生成补证清单（只读）"}</button></header>
          {evidenceGaps?.leads.length ? <div>{evidenceGaps.leads.slice(0,8).map((lead)=><article className={evidenceGapShortlist.includes(lead.id)?"selected":""} key={lead.id}><div><small>账号匹配 {lead.accountFitScore} · {lead.sourceId} · {lead.ageHours} 小时前 · 查询 {lead.queryLanguage} · {lead.qualityBoundary.qualityBound?"质量证据已绑定":"质量证据待复核"}</small><b>{lead.title}</b><span>还缺 {lead.missingIndependentSources} 个独立来源 · 建议检索：{lead.suggestedQueries[1]??lead.suggestedQueries[0]}</span></div><button type="button" onClick={()=>toggleEvidenceGap(lead.id)}>{evidenceGapShortlist.includes(lead.id)?"移出清单":"加入补证"}</button></article>)}</div> : <p>{evidenceGaps?"当前没有同时满足七天时效和账号匹配门槛的单来源线索。":"这里只筛出值得继续找第二来源的线索；不会自动搜索，也不会把线索当成已核验选题。"}</p>}
          ...
          {evidenceMetadataPreview?.status==="no_metadata_candidates"&&evidenceGapShortlist.length===1&&evidenceSearchPlan?.readyForHumanResearchReview&&<aside className="manualEvidenceHandoff"><div><small>MANUAL EVIDENCE HANDOFF</small><b>RSS 暂无合格第二来源，转人工公开页补证</b><span>已预选当前唯一线索。按钮只填写已登记来源名称；仍需你粘贴公开 HTTPS 文章链接、标题、发布时间并声明发布者身份。</span></div><div>{manualHandoffSourceSuggestions.map((source)=><button type="button" key={source.id} onClick={()=>updateManualEvidenceDraft("sourceName",source.name)}>使用 {source.aliases[0]??source.name}</button>)}</div><footer>不会自动打开、抓取或保存 · 正文读取 0 · 事实核验 0 · 来源锁 0</footer></aside>}
          {evidenceSearchPlan?.readyForHumanResearchReview&&<aside className="manualEvidenceReadiness"><header><b>人工补证必填进度 {manualEvidenceFormReadiness.completed}/{manualEvidenceFormReadiness.total}</b><span>{describeManualEvidencePreviewReadiness(manualEvidenceFormReadiness.ready,manualEvidenceTextAssessment.blocksPreview||manualSourceLinkHostAssessment.blocksPreview||manualEvidencePublishedAtAssessment.blocksPreview)}</span></header><div>{manualEvidenceFormReadiness.items.map((item)=><span className={item.complete?"complete":"missing"} key={item.id}><i>{item.complete?"✓":"○"}</i>{item.label}</span>)}</div><footer>这里只检查是否填写 · 不请求链接 · 不保存 · 不核验事实</footer></aside>}
          {evidenceSearchPlan?.readyForHumanResearchReview&&<aside className="manualEvidenceForm"><header><b>人工公开来源预览（不保存）</b><span>当前仅支持一次审查 1 个标题；修改任一字段会清除旧预览</span></header><div><label>待补证标题<select value={manualEvidenceDraft.leadId} onChange={(event)=>updateManualEvidenceDraft("leadId",event.target.value)}><option value="">请选择</option>{evidenceGaps?.leads.filter((lead)=>evidenceGapShortlist.includes(lead.id)).map((lead)=><option key={lead.id} value={lead.id}>{lead.title}</option>)}</select></label><label>来源名称<input list="manualEvidenceSourceOptions" value={manualEvidenceDraft.sourceName} maxLength={80} onChange={(event)=>updateManualEvidenceDraft("sourceName",event.target.value)}/><datalist id="manualEvidenceSourceOptions">{manualSourceNameSuggestions.map((source)=><option key={source.id} value={source.name}>{source.aliases.join(" / ")}</option>)}</datalist><span>仅提供已登记的名称建议；不会自动抓取公众号</span>{manualEvidenceTextAssessment.sourceName.message&&<span className="sourceHostHint error" role="alert" aria-live="polite">{manualEvidenceTextAssessment.sourceName.message}</span>}{manualSourceLinkHostAssessment.message&&<span className={manualSourceLinkHostAssessment.blocksPreview?"sourceHostHint error":"sourceHostHint"} role={manualSourceLinkHostAssessment.blocksPreview?"alert":undefined} aria-live="polite">{manualSourceLinkHostAssessment.message}</span>}</label><label>发布者身份<select value={manualEvidenceDraft.publisherRole} onChange={(event)=>updateManualEvidenceDraft("publisherRole",event.target.value as ManualEvidenceDraft["publisherRole"])}><option value="">请选择</option><option value="original_publisher">原始发布者</option><option value="syndicated_or_repost">转载页 / 聚合页</option></select></label><label>候选标题<input value={manualEvidenceDraft.title} maxLength={300} onChange={(event)=>updateManualEvidenceDraft("title",event.target.value)}/>{manualEvidenceTextAssessment.title.message&&<span className="sourceHostHint error" role="alert" aria-live="polite">{manualEvidenceTextAssessment.title.message}</span>}</label><label>公开 HTTPS 链接<input type="url" value={manualEvidenceDraft.canonicalUrl} maxLength={2048} onChange={(event)=>updateManualEvidenceDraft("canonicalUrl",event.target.value)}/></label><label>发布时间（ISO 8601）<input value={manualEvidenceDraft.publishedAt} maxLength={40} placeholder="2026-08-23T08:00:00Z" onChange={(event)=>updateManualEvidenceDraft("publishedAt",event.target.value)}/>{manualEvidencePublishedAtAssessment.message&&<span className={manualEvidencePublishedAtAssessment.blocksPreview?"sourceHostHint error":"sourceHostHint"} role={manualEvidencePublishedAtAssessment.blocksPreview?"alert":undefined} aria-live="polite">{manualEvidencePublishedAtAssessment.message}</span>}</label></div><button type="button" disabled={manualEvidenceBusy||!manualEvidenceFormReadiness.ready||manualEvidenceTextAssessment.blocksPreview||manualSourceLinkHostAssessment.blocksPreview||manualEvidencePublishedAtAssessment.blocksPreview||evidenceGapShortlist.length!==1} onClick={previewManualEvidence}>{manualEvidenceBusy?"校验中…":"预览人工来源（不保存）"}</button>{manualEvidencePreview?.readyForHumanEvidenceReview&&manualEvidencePreview.targets?.flatMap((target)=>target.candidates.map((candidate)=><section key={candidate.id}><b>{candidate.title}</b><span>{candidate.publisherRole==="original_publisher"?"原始发布者":"转载页"} · {target.originalHost} → {candidate.candidateHost} · 较原来源 {candidate.publishedDeltaHours>0?"+":""}{candidate.publishedDeltaHours} 小时</span>{target.originalEvidence&&<a href={target.originalEvidence.canonicalUrl} target="_blank" rel="noreferrer">人工打开原始来源</a>}<a href={candidate.canonicalUrl} target="_blank" rel="noreferrer">人工打开候选来源</a><button type="button" onClick={()=>selectManualEvidenceCandidate(target.leadId,candidate.id)}>进入六项审查预览</button></section>))}{manualEvidencePreview&&<footer>{manualEvidencePreview.readyForHumanEvidenceReview?`已接受 ${manualEvidencePreview.summary?.candidatesAccepted??0} 条临时候选；可进入审查但不能保存来源锁`:`已阻断：${manualEvidencePreview.blockers.map(formatManualEvidenceBlocker).join(" / ")}`} · 候选 URL 请求 0 · 保存 0 · 来源锁 0</footer>}</aside>}
          {evidenceMetadataPreview&&<div className="evidenceMetadataResults">{evidenceMetadataPreview.targets.flatMap((target)=>target.candidates.map((candidate)=><article className={evidenceReviewDecisions[target.leadId]?.candidateId===candidate.id?"selected":""} key={`${target.leadId}:${candidate.id}`}><div><small>待人工判断 · {candidate.sourceName} · 标题相似度 {candidate.titleSimilarity} · 较原来源 {candidate.publishedDeltaHours>0?"+":""}{candidate.publishedDeltaHours} 小时 · 候选质量证据已绑定</small><b>{candidate.title}</b><span>原来源 {target.originalHost??"主机未知"} → 候选 {candidate.candidateHost??"主机未知"} · 共同词项：{candidate.sharedTerms.join(" / ")} · 质量指纹 {candidate.candidateQualityEvidenceFingerprint.slice(0,12)}</span></div><div>{target.originalEvidence&&<a href={target.originalEvidence.canonicalUrl} target="_blank" rel="noreferrer">查看原来源</a>}<a href={candidate.canonicalUrl} target="_blank" rel="noreferrer">查看候选来源</a><button type="button" onClick={()=>selectEvidenceCandidate(target.leadId,candidate.id)}>{evidenceReviewDecisions[target.leadId]?.candidateId===candidate.id?"已选择":"选择候选"}</button></div></article>))}{evidenceMetadataPreview.status==="metadata_preview_blocked"&&<p>已阻断：上游质量证据链已失效，请重新生成补证清单。</p>}{evidenceMetadataPreview.status!=="metadata_preview_blocked"&&evidenceMetadataPreview.summary.candidatesReturned===0&&<p>本轮公开 RSS 元数据没有达到宽松候选阈值；不把“没有找到”解释为事件不存在。<small>候选排除诊断：来源范围 {evidenceMetadataPreview.candidateAudit.excludedBySourceScope} · 质量门 {evidenceMetadataPreview.candidateAudit.excludedByQualityGate} · 时间窗 {evidenceMetadataPreview.candidateAudit.excludedByTimeWindow} · 标题匹配 {evidenceMetadataPreview.candidateAudit.excludedByTitleMatch} · 限额省略 {evidenceMetadataPreview.candidateAudit.omittedByPerTargetLimit}</small></p>}{evidenceMetadataPreview.summary.candidatesReturned>0&&<footer>候选质量链 {evidenceMetadataPreview.summary.candidatesWithQualityEvidence}/{evidenceMetadataPreview.summary.candidatesReturned} · 预览指纹 {evidenceMetadataPreview.qualityBoundary.previewQualityFingerprint?.slice(0,16)??"无"} · 排除：来源 {evidenceMetadataPreview.candidateAudit.excludedBySourceScope} / 质量 {evidenceMetadataPreview.candidateAudit.excludedByQualityGate} / 时间 {evidenceMetadataPreview.candidateAudit.excludedByTimeWindow} / 标题 {evidenceMetadataPreview.candidateAudit.excludedByTitleMatch} · 正文读取 0 · 事实核验 0</footer>}</div>}
          {Object.keys(evidenceReviewDecisions).length>0&&<aside className="evidenceReviewForm"><header><b>人工证据审查（本次页面临时状态）</b><span>{evidenceReviewPreview?.readyForAuthorizedSourceLockSave?"预览通过，可生成不保存计划":evidenceReviewPreview?.humanEvidenceReviewComplete?"审查预览完成，但仍有下游阻塞":"完成六项判断后只生成预览"}</span></header><ManualEvidenceReviewHandoff decisions={evidenceReviewDecisions} preview={evidenceReviewPreview}/>{Object.entries(evidenceReviewDecisions).map(([leadId,decision])=><div key={leadId}><strong>{evidenceMetadataPreview?.targets.find((target)=>target.leadId===leadId)?.candidates.find((candidate)=>candidate.id===decision.candidateId)?.title??manualEvidencePreview?.targets?.find((target)=>target.leadId===leadId)?.candidates.find((candidate)=>candidate.id===decision.candidateId)?.title}</strong><span>人工核验进度：{Object.values(decision.checks).filter(Boolean).length}/{evidenceReviewChecklist.length}</span><span>待确认：{evidenceReviewChecklist.filter(({id})=>!decision.checks[id]).map(({label})=>label).join(" / ")||"无"}</span>{decision.candidateMode==="manual_public_metadata"&&<span>发布者身份：{formatManualEvidencePublisherRole(manualEvidencePreview?.targets?.find((target)=>target.leadId===leadId)?.candidates.find((candidate)=>candidate.id===decision.candidateId)?.publisherRole)}</span>}{decision.candidateMode==="rss_metadata"&&<MetadataEvidenceReviewLinks preview={evidenceMetadataPreview} leadId={leadId} candidateId={decision.candidateId}/>}{decision.candidateMode==="manual_public_metadata"&&<ManualEvidenceReviewLinks preview={manualEvidencePreview} leadId={leadId} candidateId={decision.candidateId}/>}{evidenceReviewChecklist.map(({id,label})=><label key={id}><input type="checkbox" checked={decision.checks[id]} onChange={()=>toggleEvidenceReviewCheck(leadId,id)}/>{label}</label>)}</div>)}<div className="evidenceReviewActions"><button type="button" disabled={evidenceReviewBusy} onClick={previewEvidenceReview}>{evidenceReviewBusy?"核对中…":"预览证据审查（不保存）"}</button><button type="button" disabled={!evidenceReviewPreview?.readyForAuthorizedSourceLockSave||sourceLockSavePlanBusy} onClick={previewSourceLockSavePlan}>{sourceLockSavePlanBusy?"绑定中…":"生成来源锁保存计划（不保存）"}</button><button type="button" disabled={!sourceLockSavePlan?.readyForAuthorizationRequest||sourceLockSaveAuthorizationPreviewBusy} onClick={previewSourceLockSaveAuthorization}>{sourceLockSaveAuthorizationPreviewBusy?"生成中…":"预览单次保存授权（不授权）"}</button></div>{sourceLockSavePlan&&<SourceLockPlanSummary plan={sourceLockSavePlan}/>} {sourceLockSavePlan&&<SourceLockStorageReadinessCard readiness={sourceLockStorageReadiness} busy={sourceLockStorageReadinessBusy} onInspect={inspectSourceLockStorageReadiness}/>} {sourceLockSaveAuthorizationPreview&&<SourceLockAuthorizationTerms preview={sourceLockSaveAuthorizationPreview}/>} {sourceLockSavePlan&&<SourceLockSaveBoundarySummary plan={sourceLockSavePlan} preview={sourceLockSaveAuthorizationPreview} readiness={sourceLockStorageReadiness}/>} {evidenceReviewPreview&&<footer>合格 {evidenceReviewPreview.summary.targetsEligible}/{evidenceReviewPreview.summary.targetsRequired} · 审查指纹 {evidenceReviewPreview.reviewFingerprint?.slice(0,16)??"未生成"}{evidenceReviewPreview.blockers.length?` · 需修正 ${evidenceReviewPreview.blockers.map(formatEvidenceReviewBlocker).join(" / ")}`:""} · 下游阻塞 {evidenceReviewPreview.downstreamBlockers.join(" / ")||"无"} · 来源锁仍为 0</footer>}{sourceLockSavePlan&&<footer>计划记录 {sourceLockSavePlan.plannedRecordCount} · 计划指纹 {sourceLockSavePlan.savePlanFingerprint?.slice(0,16)??"未生成"}{sourceLockSavePlan.blockers.length?` · 需修正 ${sourceLockSavePlan.blockers.map(formatSourceLockPlanBlocker).join(" / ")}`:""} · 授权未授予 · 写库 0</footer>}{sourceLockSavePlan&&sourceLockSaveAuthorizationPreview&&<footer>授权预览 {sourceLockSaveAuthorizationPreview.authorizationPreviewFingerprint?.slice(0,16)??"未生成"}{sourceLockSaveAuthorizationPreview.blockers.length?` · 需修正 ${sourceLockSaveAuthorizationPreview.blockers.map(formatSourceLockAuthorizationBlocker).join(" / ")}`:""} · 确认短语仅供核对：<code>{sourceLockSaveAuthorizationPreview.requiredConfirmation??"无"}</code> · 未授权 · 保存 0</footer>}</aside>}
          <footer>质量链 {evidenceGaps?.qualityBoundary.allReturnedLeadsQualityBound?"已完整绑定":"未形成"} · 指纹 {evidenceGaps?.qualityBoundary.shortlistQualityFingerprint?.slice(0,16)??"无"} · 本次临时选择 {evidenceGapShortlist.length} · RSS 元数据候选 {evidenceMetadataPreview?.summary.candidatesReturned??0} · 审查保存 0 · 事实核验 0 · 来源锁 0 · 草稿解锁 0 · 发布 0</footer>
          {sourceLockSavePlan&&sourceLockSaveAuthorizationPreview&&<section className="sourceLockSaveControl"><header><div><small>SOURCE LOCK SAVE · HUMAN CONFIRMATION</small><b>保存当前人工复核的来源锁</b><span>只写入本机 D1；不会自动核验事实、生成草稿、打开平台或发布</span></div></header><label>请输入完整确认短语<input value={sourceLockSaveConfirmation} onChange={(event)=>{setSourceLockSaveConfirmation(event.target.value);setSourceLockSaveResult(null)}} placeholder={sourceLockSaveAuthorizationPreview.requiredConfirmation??"等待授权预览"}/></label><button type="button" disabled={sourceLockSaveBusy||!sourceLockSavePlan.readyForAuthorizationRequest||!sourceLockSaveAuthorizationPreview.eligibleForExplicitSourceLockSaveAuthorization||sourceLockStorageReadiness?.storage?.verified!==true||sourceLockSaveConfirmation!==sourceLockSaveAuthorizationPreview.requiredConfirmation} onClick={saveReviewedSourceLock}>{sourceLockSaveBusy?"保存中…":"确认并保存 1 条来源锁"}</button>{sourceLockSaveResult&&<p className={sourceLockSaveResult.persisted||sourceLockSaveResult.alreadyPersisted?"success":"blocked"}>{sourceLockSaveResult.persisted?`已保存来源锁 ${sourceLockSaveResult.sourceLockId??""}`:sourceLockSaveResult.alreadyPersisted?"当前来源锁已保存，无重复写入":`保存被阻断：${sourceLockSaveResult.blockers.join(" · ")}`}</p>}<footer>需保存计划、授权预览、表结构与确认短语全部匹配 · 草稿解锁 {sourceLockSaveResult?.draftsUnlocked??0} · 发布 {sourceLockSaveResult?.publishTriggered?"已触发":"0"}</footer></section>}
        </section>
        <section className="controlStrip">
          ...
          <button className="generate" disabled={busy} onClick={queueGeneration}>{busy?"创建中…":"立即生成草稿雏形 →"}</button>
        </section>
        ...
        <nav className="topicPager" aria-label="热点翻页"><button type="button" disabled={ideaPage===1} onClick={()=>setIdeaPage((page)=>Math.max(1,page-1))}>← 上一页</button><span>{ideaPage} / {ideaPageCount}</span><button type="button" disabled={ideaPage===ideaPageCount} onClick={()=>setIdeaPage((page)=>Math.min(ideaPageCount,page+1))}>下一页 →</button></nav>
      </>}
      ...
      {view === "production" && <section className="panel">
        ...
        {localDraftPreviews.length>0&&<section ref={localDraftPreviewRef} className="localDraftPreview"><header><div><small>DOUYIN DRAFT · REVIEW REQUIRED</small><b>抖音文章草稿雏形已生成</b></div><span>{localDraftPreviews.every((draft)=>draft.status==="model_generated_unverified")?"本机 AI 生成 · 待来源核验":"规则草稿 · 本机 AI 继续生成中"} · 不保存 · 不发布</span></header>{localDraftPreviews.map((draft)=><article key={draft.ideaId}><div><small>{draft.status==="model_generated_unverified"?"本机 AI 草稿":"即时规则草稿"}</small><h3>{draft.title}</h3><p>{draft.body}</p><div>{draft.hashtags.map((tag)=><span key={tag}>#{tag}</span>)}</div></div><aside><small>规则演示区间</small><b>{draft.estimatedViews}</b><em>未使用真实账号数据，不能视为预测</em><button type="button" className="douyinHandoff" disabled={draft.status!=="source_locked_review_ready"} onClick={()=>handoffLocalDraftToDouyin(draft)}>{draft.status==="source_locked_review_ready"?"自动填入抖音（不发布）":"待来源核验后填入"}</button><strong>{draft.status!=="source_locked_review_ready"?"下一步：绑定公开来源并人工核验":douyinConnectorStatus==="received"?"连接器已接收，正在抖音页填入":douyinConnectorStatus==="opening"?"正在打开抖音创作页…":douyinConnectorStatus==="unavailable"?"未检测到连接器；文案已复制，可手动粘贴":"点击后打开当前浏览器的抖音创作页"}</strong></aside></article>)}</section>}
        {reviewMigrationSingleUseAuthorizationRequest&&<section className="reviewMigrationPostApplyVerificationPlanGate"><header><div><small>POST-APPLY VERIFICATION · 只读计划</small><b>应用后校验与失败回滚证明</b><span>只锁定未来核验合同；当前不接收授权、不执行 SQL</span></div><button type="button" disabled={reviewMigrationPostApplyVerificationPlanBusy} onClick={previewReviewMigrationPostApplyVerificationPlan}>{reviewMigrationPostApplyVerificationPlanBusy?"锁定中…":"预览应用后只读校验（仍不执行）"}</button></header>{reviewMigrationPostApplyVerificationPlan&&<aside className={reviewMigrationPostApplyVerificationPlan.readyForFuturePostApplyVerification?"reviewMigrationPostApplyVerificationPlan ready":"reviewMigrationPostApplyVerificationPlan blocked"}><b>{reviewMigrationPostApplyVerificationPlan.readyForFuturePostApplyVerification?"应用后核验合同已锁定，尚未执行":"应用后核验计划被阻断"}</b><span>{reviewMigrationPostApplyVerificationPlan.expectedCounts.tables} 表 · {reviewMigrationPostApplyVerificationPlan.expectedCounts.indexes} 索引 · {reviewMigrationPostApplyVerificationPlan.expectedCounts.columns} 字段 · {reviewMigrationPostApplyVerificationPlan.expectedCounts.metadataQueries} 次只读元数据查询</span><span>迁移日志 0009 + 0010 · 业务行读取 0</span>{reviewMigrationPostApplyVerificationPlan.verificationPlanFingerprint&&<span>核验计划指纹 <code>{reviewMigrationPostApplyVerificationPlan.verificationPlanFingerprint}</code></span>}<span>回滚证据：仅在未来授权事务失败时生成；当前不存在</span><em>授权 0 · SQL 执行 0 · 数据库读取 0 · 数据库写入 0 · 回滚执行 0</em></aside>}</section>}
        <section className={unifiedDraftPackagePlan?.status==="platform_text_unified_draft_package_plan_ready"?"unifiedDraftPackagePlan ready":"unifiedDraftPackagePlan blocked"}><header><div><small>XIAOHONGSHU + DOUYIN · 只读输入检查</small><b>{unifiedDraftPackagePlan?.status==="platform_text_unified_draft_package_plan_ready"?"统一草稿包输入已完整，仍需独立打开授权":unifiedDraftPackagePlan?"统一草稿包仍有上游阻塞":"尚未检查统一草稿包输入"}</b></div><button type="button" disabled={unifiedDraftPackagePlanBusy} onClick={previewUnifiedDraftPackagePlan}>{unifiedDraftPackagePlanBusy?"检查中…":"检查双平台草稿包（不会打开平台）"}</button></header><div><article><i>{unifiedDraftPackagePlan?.copyHandoffReady?"✓":"×"}</i><b>双平台文案</b><span>{unifiedDraftPackagePlan?.copyHandoffReady?"指纹一致":"等待已审核文案交接"}</span></article><article><i>{unifiedDraftPackagePlan?.reviewedAssetReferencesReady?"✓":"×"}</i><b>视觉素材</b><span>{unifiedDraftPackagePlan?.reviewedAssetReferencesReady?`${unifiedDraftPackagePlan.assetCount} 项已审素材引用`:"等待 SVG 与人工视觉审核"}</span></article><article><i>{unifiedDraftPackagePlan?.draftPackageInputsReady?"✓":"×"}</i><b>统一封包</b><span>{unifiedDraftPackagePlan?.draftPackageInputsReady?`${unifiedDraftPackagePlan.platformCount} 个平台输入完整`:"不生成虚假草稿包"}</span></article><article><i>{unifiedDraftPackagePlan?.eligibleForCreatorPageOpenAuthorization?"✓":"—"}</i><b>创作页授权</b><span>{unifiedDraftPackagePlan?.eligibleForCreatorPageOpenAuthorization?"可另行请求本人授权":"保持锁定"}</span></article></div><footer><span>{unifiedDraftPackagePlan?.blockers.length?unifiedDraftPackagePlan.blockers.map((blocker)=>unifiedDraftPackageBlockerLabels[blocker]??blocker).join(" · "):unifiedDraftPackagePlan?"输入阻塞 0；本卡仍不执行后续动作":"点击后只核对输入链，不读取账号状态"}</span><em>浏览器打开 0 · 登录 0 · 上传 0 · 草稿保存 0 · 发布 0</em></footer></section>
        <section className={durableReviewInputs?.inputsReady?"durableReviewInputs ready":"durableReviewInputs blocked"}><header><div><small>D1 REVIEW RECEIPTS · 只读</small><b>{durableReviewInputs?.inputsReady?"两类人工审核记录均已持久化":"读取真实文案与视觉审核记录"}</b><span>只查询审核收据摘要；不会保存这里输入的指纹</span></div><button type="button" disabled={durableReviewInputsBusy} onClick={checkDurableReviewInputs}>{durableReviewInputsBusy?"查询中…":"查询 D1 审核记录（只读）"}</button></header><aside className="durableReviewCatalog"><div><b>不知道指纹在哪里？</b><span>从 D1 只读列出最近 5 条有效收据，再由你选择；不会自动配对。</span></div><button type="button" disabled={reviewReceiptCatalogBusy} onClick={discoverReviewReceipts}>{reviewReceiptCatalogBusy?"查找中…":"查找最近审核记录（只读）"}</button>{reviewReceiptCatalog&&<section>{reviewReceiptCatalog.catalogReadReady?<><div><strong>文案审核</strong>{reviewReceiptCatalog.draftReviews.length?reviewReceiptCatalog.draftReviews.map((entry)=><button type="button" key={entry.fingerprint} title={entry.fingerprint} onClick={()=>{setDraftReviewFingerprint(entry.fingerprint);setDurableReviewInputs(null)}}>{entry.fingerprint.slice(0,12)}… · {entry.createdAt}</button>):<span>存储可读，当前没有真实持久化记录</span>}</div><div><strong>视觉审核</strong>{reviewReceiptCatalog.visualReviews.length?reviewReceiptCatalog.visualReviews.map((entry)=><button type="button" key={entry.fingerprint} title={entry.fingerprint} onClick={()=>{setVisualReviewFingerprint(entry.fingerprint);setDurableReviewInputs(null)}}>{entry.fingerprint.slice(0,12)}… · {entry.createdAt}</button>):<span>存储可读，当前没有真实持久化记录</span>}</div></>:<div className="catalogUnavailable"><strong>目录暂不可读</strong><span>{reviewReceiptCatalog.blockers.map((blocker)=>reviewReceiptCatalogBlockerLabels[blocker]??blocker).join(" · ")}</span></div>}<em>数据库读取 {reviewReceiptCatalog.databaseReads} · 写入 0 · 自动配对 0</em></section>}<button className="inspectReviewStorage" type="button" disabled={reviewStorageReadinessBusy} onClick={inspectReviewStorage}>{reviewStorageReadinessBusy?"诊断中…":"诊断审核表结构（只读）"}</button>{reviewStorageReadiness&&<section className="reviewStorageReadiness"><div><strong>文案审核表 · 0009</strong><span>{reviewStorageReadiness.draftReviewStorage?.status??"不可用"} · 缺对象 {reviewStorageReadiness.draftReviewStorage?.missingObjectCount??0} · 缺字段 {reviewStorageReadiness.draftReviewStorage?.missingColumnCount??0}</span></div><div><strong>视觉审核表 · 0010</strong><span>{reviewStorageReadiness.visualReviewStorage?.status??"不可用"} · 缺对象 {reviewStorageReadiness.visualReviewStorage?.missingObjectCount??0} · 缺字段 {reviewStorageReadiness.visualReviewStorage?.missingColumnCount??0}</span></div><em>只读元数据查询 {reviewStorageReadiness.databaseReads} · 迁移执行 0 · 数据库写入 0</em><button type="button" disabled={reviewMigrationAuthorizationPreviewBusy} onClick={previewReviewMigrationAuthorization}>{reviewMigrationAuthorizationPreviewBusy?"生成中…":"预览迁移授权范围（不执行）"}</button>{reviewMigrationAuthorizationPreview&&<aside className="reviewMigrationAuthorizationPreview"><b>{reviewMigrationAuthorizationPreview.eligibleForExplicitLocalMigrationAuthorization?"授权范围已锁定，但尚未授权":"授权范围预览被阻断"}</b><span>仅本机 DB · 0009 + 0010 · {reviewMigrationAuthorizationPreview.tableCount} 表 · {reviewMigrationAuthorizationPreview.indexCount} 索引 · 远程 0</span>{reviewMigrationAuthorizationPreview.migrationManifest.length>0&&<div className="reviewMigrationManifest">{reviewMigrationAuthorizationPreview.migrationManifest.map((migration)=><details key={migration.tag} open><summary>{migration.tag} · {migration.tables.length} 表 + {migration.indexes.length} 索引</summary><span>表：{migration.tables.join(" · ")}</span><span>索引：{migration.indexes.join(" · ")}</span></details>)}</div>}{reviewMigrationAuthorizationPreview.requiredConfirmation&&<code>{reviewMigrationAuthorizationPreview.requiredConfirmation}</code>}<em>清单只读 · SQL 不执行 · 命令准备 0 · 执行器连接 0 · 迁移执行 0 · 数据库写入 0</em></aside>}<button type="button" disabled={reviewMigrationIsolatedRehearsalBusy} onClick={rehearseReviewMigrationsInMemory}>{reviewMigrationIsolatedRehearsalBusy?"演练中…":"隔离演练 0009/0010（仅内存）"}</button>{reviewMigrationIsolatedRehearsal&&<aside className={reviewMigrationIsolatedRehearsal.successPathVerified&&reviewMigrationIsolatedRehearsal.failurePathVerified?"reviewMigrationIsolatedRehearsal verified":"reviewMigrationIsolatedRehearsal failed"}><b>{reviewMigrationIsolatedRehearsal.successPathVerified&&reviewMigrationIsolatedRehearsal.failurePathVerified?"隔离成功路径与失败回滚均通过":"隔离演练未通过"}</b><span>0009/0010 已隔离应用 {reviewMigrationIsolatedRehearsal.appliedTags.length}/2 · 结构 {reviewMigrationIsolatedRehearsal.tableCount} 表 + {reviewMigrationIsolatedRehearsal.indexCount} 索引</span><span>故障探针 {reviewMigrationIsolatedRehearsal.rollbackVerifiedCount}/{reviewMigrationIsolatedRehearsal.rollbackScenarioCount} 已回滚 · 内存写入 {reviewMigrationIsolatedRehearsal.ephemeralDatabaseWrites?"有":"0"}</span><em>真实 D1 访问 0 · 真实 D1 写入 0 · 正式迁移执行 0 · 业务结果 0</em></aside>}<button type="button" disabled={reviewMigrationExecutionPreflightBusy||!reviewMigrationAuthorizationPreview||!reviewMigrationIsolatedRehearsal} onClick={previewReviewMigrationExecutionPreflight}>{reviewMigrationExecutionPreflightBusy?"检查中…":"检查执行前状态（不授权）"}</button>{reviewMigrationExecutionPreflight&&<aside className={reviewMigrationExecutionPreflight.readyForExplicitAuthorizationRequest?"reviewMigrationExecutionPreflight ready":"reviewMigrationExecutionPreflight blocked"}><b>{reviewMigrationExecutionPreflight.readyForExplicitAuthorizationRequest?"执行前证据就绪，等待明确授权":"执行前检查被阻断"}</b><span>{reviewMigrationExecutionPreflight.evidenceValidated?"清单指纹 + 隔离演练均已校验":reviewMigrationExecutionPreflight.blockers.join(" · ")}</span>{reviewMigrationExecutionPreflight.migrationExecutionPreflightFingerprint&&<span>执行前指纹 <code>{reviewMigrationExecutionPreflight.migrationExecutionPreflightFingerprint}</code></span>}{reviewMigrationExecutionPreflight.requiredConfirmation&&<span>仅供你确认；本轮未收到确认：<code>{reviewMigrationExecutionPreflight.requiredConfirmation}</code></span>}<em>确认收到 0 · 授权 0 · 执行器 0 · 正式迁移 0 · 真实 D1 写入 0</em></aside>}<button type="button" disabled={reviewMigrationLocalTargetDiagnosticBusy||!reviewMigrationExecutionPreflight} onClick={diagnoseReviewMigrationLocalTarget}>{reviewMigrationLocalTargetDiagnosticBusy?"诊断中…":"诊断本机 D1 目标（只读）"}</button>{reviewMigrationLocalTargetDiagnostic&&<aside className={reviewMigrationLocalTargetDiagnostic.localTargetProven?"reviewMigrationLocalTargetDiagnostic ready":"reviewMigrationLocalTargetDiagnostic blocked"}><b>{reviewMigrationLocalTargetDiagnostic.localTargetProven?"本机 D1 目标已锁定，仍待明确授权":"本机 D1 目标诊断被阻断"}</b><span>{reviewMigrationLocalTargetDiagnostic.localTargetProven?`${reviewMigrationLocalTargetDiagnostic.targetBinding} · Miniflare 回环环境 · 0009/0010 已登记`:reviewMigrationLocalTargetDiagnostic.blockers.join(" · ")}</span>{reviewMigrationLocalTargetDiagnostic.localTargetEvidenceFingerprint&&<span>目标证据指纹 <code>{reviewMigrationLocalTargetDiagnostic.localTargetEvidenceFingerprint}</code></span>}<span>本机数据库候选 {reviewMigrationLocalTargetDiagnostic.localStateCandidateCount} · 远程目标未连接</span><em>数据库文件打开 0 · 数据库读取 0 · 数据库写入 0 · 命令准备 0 · 授权 0</em></aside>}<button type="button" disabled={reviewMigrationSingleUseAuthorizationRequestBusy||!reviewMigrationLocalTargetDiagnostic} onClick={previewReviewMigrationSingleUseAuthorizationRequest}>{reviewMigrationSingleUseAuthorizationRequestBusy?"锁定中…":"生成单次授权请求（仍不授权）"}</button>{reviewMigrationSingleUseAuthorizationRequest&&<aside className={reviewMigrationSingleUseAuthorizationRequest.readyForHumanConfirmation?"reviewMigrationSingleUseAuthorizationRequest ready":"reviewMigrationSingleUseAuthorizationRequest blocked"}><b>{reviewMigrationSingleUseAuthorizationRequest.readyForHumanConfirmation?"单次授权请求已锁定，等待本人精确确认":"单次授权请求预览被阻断"}</b>{reviewMigrationSingleUseAuthorizationRequest.sourceMigrationScopeFingerprint&&<span>迁移范围指纹 <code>{reviewMigrationSingleUseAuthorizationRequest.sourceMigrationScopeFingerprint}</code></span>}{reviewMigrationSingleUseAuthorizationRequest.migrationExecutionPreflightFingerprint&&<span>执行前指纹 <code>{reviewMigrationSingleUseAuthorizationRequest.migrationExecutionPreflightFingerprint}</code></span>}{reviewMigrationSingleUseAuthorizationRequest.localTargetEvidenceFingerprint&&<span>本机目标指纹 <code>{reviewMigrationSingleUseAuthorizationRequest.localTargetEvidenceFingerprint}</code></span>}{reviewMigrationSingleUseAuthorizationRequest.authorizationRequestFingerprint&&<span>授权请求指纹 <code>{reviewMigrationSingleUseAuthorizationRequest.authorizationRequestFingerprint}</code></span>}{reviewMigrationSingleUseAuthorizationRequest.requiredConfirmation&&<span>未来仅接受精确文本；此页面不接收：<code>{reviewMigrationSingleUseAuthorizationRequest.requiredConfirmation}</code></span>}<span>未来票据 单次 · 接受后 5 分钟 · 失败回滚 · 应用后只读复核</span><em>确认收到 0 · 票据签发 0 · 执行契约 0 · 命令准备 0 · 数据库写入 0</em></aside>}</section>}</aside><div className="durableReviewFields"><label>文案审核指纹<input value={draftReviewFingerprint} onChange={(event)=>{setDraftReviewFingerprint(event.target.value.trim());setDurableReviewInputs(null)}} maxLength={64} autoComplete="off" spellCheck={false} placeholder="64 位 review fingerprint" /></label><label>视觉审核指纹<input value={visualReviewFingerprint} onChange={(event)=>{setVisualReviewFingerprint(event.target.value.trim());setDurableReviewInputs(null)}} maxLength={64} autoComplete="off" spellCheck={false} placeholder="64 位 visual review fingerprint" /></label></div><div className="durableReviewSummary"><span><b>{durableReviewInputs?.draftReview?.found?"✓":"—"}</b>文案审核 · {durableReviewInputs?.draftReview?.reviewedPlatforms??0} 平台</span><span><b>{durableReviewInputs?.visualReview?.found?"✓":"—"}</b>视觉审核 · {durableReviewInputs?.visualReview?.reviewedAssets??0} 素材</span><span><b>{durableReviewInputs?.databaseReads??0}</b>数据库读取</span><span><b>0</b>数据库写入</span></div><footer><span>{durableReviewInputs?.blockers.length?durableReviewInputs.blockers.map((blocker)=>durableReviewInputBlockerLabels[blocker]??blocker).join(" · "):durableReviewInputs?"审核输入已就绪；统一封包仍需重新绑定完整上游指纹链":"请输入已保存审核记录返回的两个指纹"}</span><em>正文不返回 · 路径不返回 · 外部请求 0 · 发布 0</em></footer></section>
      ...
      {view === "accounts" && <section className="panel">
        ...
        <section className={runtimeStatus.current?"runtimeStatus current":"runtimeStatus blocked"}><header><div><small>LOCAL RUNTIME · 只读体检</small><b>{runtimeStatus.current?"本机工作流基础服务已就绪":runtimeStatus.status==="bridge_stale"?"全部服务在线，桥接版本待更新":runtimeStatus.status==="models_missing"?"Ollama 在线，中文模型尚未就绪":"存在本机服务离线"}</b></div><em>{runtimeStatus.services.filter(service=>service.online).length}/{runtimeStatus.services.length||5} 在线</em></header><div>{runtimeStatus.services.map(service=><span className={service.online&&(service.id!=="ollama"||service.modelReady!==false)?"online":"offline"} key={service.id}><i>{service.online&&(service.id!=="ollama"||service.modelReady!==false)?"✓":"!"}</i><b>{runtimeServiceLabels[service.id]??service.id}</b><small>{!service.online?"未连接":service.id==="ollama"&&service.modelReady===false?"服务在线 · 缺少 qwen3:4b":`HTTP ${service.statusCode}`}</small></span>)}</div><footer><span>{runtimeStatus.nextAction==="none"?"无需处理":runtimeStatus.nextAction==="close_old_studio_and_rerun_launcher"?"关闭旧操作台后重新运行启动器":runtimeStatus.nextAction==="install_or_start_ollama"?"安装或启动 Ollama 中文改写模型":runtimeStatus.nextAction==="pull_qwen3_4b"?"下载 qwen3:4b 中文改写模型":"运行本机启动器"}</span><em>重启 0 · 下载 0 · 模型调用 0 · 发布 0</em></footer></section>
