...
type LocalDraftPreview = { ideaId:string; title:string; body:string; hashtags:string[]; estimatedViews:string; status:"local_preview"|"model_generated_unverified" };
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      {view === "production" && <section className="panel">
        ...
        {localDraftPreviews.length>0&&<section ref={localDraftPreviewRef} className="localDraftPreview"><header><div><small>DOUYIN DRAFT · CONNECTOR READY</small><b>抖音文章草稿雏形已生成</b></div><span>{localDraftPreviews.every((draft)=>draft.status==="model_generated_unverified")?"本机 AI 生成 · 待来源核验":"规则草稿 · 本机 AI 继续生成中"} · 不保存 · 不发布</span></header>{localDraftPreviews.map((draft)=><article key={draft.ideaId}><div><small>{draft.status==="model_generated_unverified"?"本机 AI 草稿":"即时规则草稿"}</small><h3>{draft.title}</h3><p>{draft.body}</p><div>{draft.hashtags.map((tag)=><span key={tag}>#{tag}</span>)}</div></div><aside><small>规则演示区间</small><b>{draft.estimatedViews}</b><em>未使用真实账号数据，不能视为预测</em><button type="button" className="douyinHandoff" onClick={()=>handoffLocalDraftToDouyin(draft)}>自动填入抖音（不发布）</button><strong>{douyinConnectorStatus==="received"?"连接器已接收，正在抖音页填入":douyinConnectorStatus==="opening"?"正在打开抖音创作页…":douyinConnectorStatus==="unavailable"?"未检测到连接器；文案已复制，可手动粘贴":"点击后打开当前浏览器的抖音创作页"}</strong></aside></article>)}</section>}
