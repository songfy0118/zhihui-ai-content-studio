...
import { assessManualEvidencePublishedAt, buildManualEvidenceFormReadiness, describeManualEvidencePreviewReadiness } from "./manual-evidence-form-readiness";
    ...
    <section className="workspace">
      ...
      {view === "ideas" && <>
        ...
        <section className="evidenceGaps">
          ...
          {evidenceSearchPlan?.readyForHumanResearchReview&&<aside className="manualEvidenceReadiness"><header><b>人工补证必填进度 {manualEvidenceFormReadiness.completed}/{manualEvidenceFormReadiness.total}</b><span>{describeManualEvidencePreviewReadiness(manualEvidenceFormReadiness.ready,manualSourceLinkHostAssessment.blocksPreview||manualEvidencePublishedAtAssessment.blocksPreview)}</span></header><div>{manualEvidenceFormReadiness.items.map((item)=><span className={item.complete?"complete":"missing"} key={item.id}><i>{item.complete?"✓":"○"}</i>{item.label}</span>)}</div><footer>这里只检查是否填写 · 不请求链接 · 不保存 · 不核验事实</footer></aside>}
          {evidenceSearchPlan?.readyForHumanResearchReview&&<aside className="manualEvidenceForm"><header><b>人工公开来源预览（不保存）</b><span>当前仅支持一次审查 1 个标题；修改任一字段会清除旧预览</span></header><div><label>待补证标题<select value={manualEvidenceDraft.leadId} onChange={(event)=>updateManualEvidenceDraft("leadId",event.target.value)}><option value="">请选择</option>{evidenceGaps?.leads.filter((lead)=>evidenceGapShortlist.includes(lead.id)).map((lead)=><option key={lead.id} value={lead.id}>{lead.title}</option>)}</select></label><label>来源名称<input list="manualEvidenceSourceOptions" value={manualEvidenceDraft.sourceName} maxLength={80} onChange={(event)=>updateManualEvidenceDraft("sourceName",event.target.value)}/><datalist id="manualEvidenceSourceOptions">{manualSourceNameSuggestions.map((source)=><option key={source.id} value={source.name}>{source.aliases.join(" / ")}</option>)}</datalist><span>仅提供已登记的名称建议；不会自动抓取公众号</span>{manualSourceLinkHostAssessment.message&&<span className={manualSourceLinkHostAssessment.blocksPreview?"sourceHostHint error":"sourceHostHint"} role={manualSourceLinkHostAssessment.blocksPreview?"alert":undefined} aria-live="polite">{manualSourceLinkHostAssessment.message}</span>}</label><label>发布者身份<select value={manualEvidenceDraft.publisherRole} onChange={(event)=>updateManualEvidenceDraft("publisherRole",event.target.value as ManualEvidenceDraft["publisherRole"])}><option value="">请选择</option><option value="original_publisher">原始发布者</option><option value="syndicated_or_repost">转载页 / 聚合页</option></select></label><label>候选标题<input value={manualEvidenceDraft.title} maxLength={300} onChange={(event)=>updateManualEvidenceDraft("title",event.target.value)}/></label><label>公开 HTTPS 链接<input type="url" value={manualEvidenceDraft.canonicalUrl} maxLength={2048} onChange={(event)=>updateManualEvidenceDraft("canonicalUrl",event.target.value)}/></label><label>发布时间（ISO 8601）<input value={manualEvidenceDraft.publishedAt} maxLength={40} placeholder="2026-08-23T08:00:00Z" onChange={(event)=>updateManualEvidenceDraft("publishedAt",event.target.value)}/>{manualEvidencePublishedAtAssessment.message&&<span className={manualEvidencePublishedAtAssessment.blocksPreview?"sourceHostHint error":"sourceHostHint"} role={manualEvidencePublishedAtAssessment.blocksPreview?"alert":undefined} aria-live="polite">{manualEvidencePublishedAtAssessment.message}</span>}</label></div><button type="button" disabled={manualEvidenceBusy||!manualEvidenceFormReadiness.ready||manualSourceLinkHostAssessment.blocksPreview||manualEvidencePublishedAtAssessment.blocksPreview||evidenceGapShortlist.length!==1} onClick={previewManualEvidence}>{manualEvidenceBusy?"校验中…":"预览人工来源（不保存）"}</button>{manualEvidencePreview?.readyForHumanEvidenceReview&&manualEvidencePreview.targets?.flatMap((target)=>target.candidates.map((candidate)=><section key={candidate.id}><b>{candidate.title}</b><span>{candidate.publisherRole==="original_publisher"?"原始发布者":"转载页"} · {target.originalHost} → {candidate.candidateHost} · 较原来源 {candidate.publishedDeltaHours>0?"+":""}{candidate.publishedDeltaHours} 小时</span>{target.originalEvidence&&<a href={target.originalEvidence.canonicalUrl} target="_blank" rel="noreferrer">人工打开原始来源</a>}<a href={candidate.canonicalUrl} target="_blank" rel="noreferrer">人工打开候选来源</a><button type="button" onClick={()=>selectManualEvidenceCandidate(target.leadId,candidate.id)}>进入六项审查预览</button></section>))}{manualEvidencePreview&&<footer>{manualEvidencePreview.readyForHumanEvidenceReview?`已接受 ${manualEvidencePreview.summary?.candidatesAccepted??0} 条临时候选；可进入审查但不能保存来源锁`:`已阻断：${manualEvidencePreview.blockers.map(formatManualEvidenceBlocker).join(" / ")}`} · 候选 URL 请求 0 · 保存 0 · 来源锁 0</footer>}</aside>}
