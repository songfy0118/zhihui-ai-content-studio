...
import { buildManualEvidenceFormReadiness } from "./manual-evidence-form-readiness";
    ...
    <section className="workspace">
      ...
      {view === "ideas" && <>
        ...
        <section className="evidenceGaps">
          ...
          {evidenceSearchPlan?.readyForHumanResearchReview&&<aside className="manualEvidenceReadiness"><header><b>人工补证必填进度 {manualEvidenceFormReadiness.completed}/{manualEvidenceFormReadiness.total}</b><span>{manualEvidenceFormReadiness.ready?"字段已填齐；服务器仍会校验公开 HTTPS、同源、时间窗和标题关联":"缺少字段时不会发送预览请求"}</span></header><div>{manualEvidenceFormReadiness.items.map((item)=><span className={item.complete?"complete":"missing"} key={item.id}><i>{item.complete?"✓":"○"}</i>{item.label}</span>)}</div><footer>这里只检查是否填写 · 不请求链接 · 不保存 · 不核验事实</footer></aside>}
