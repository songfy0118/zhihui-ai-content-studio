...
export function buildChineseInternetRewritePlan(blueprintResult, { model = "qwen3:4b" } = {}) {
  ...
  const instruction = [
    ...
    "陌生技术名词不要单独放在标题最前面；先给读者熟悉的公司、行业或利益锚点，再用冲突或问题引出新名词。",
    "抖音标题优先采用一个明确停留理由：反常识、极端反差、具体数字、身份相关或强问题；数字只能来自 inputClaims。",
    "标题一次只制造一个冲突，避免堆砌热词；正文第一段立即解释标题中的关系，并明确产品归属、证据边界和仍待验证之处。",
  ...
  const plan = {
    ...
    constraints:[
      ...
      "familiar_anchor_before_unfamiliar_term",
      "single_hook_with_immediate_explanation",
